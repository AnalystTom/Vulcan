...
import { VULCAN_PRODUCTION_BUNDLE_ID } from "@vulcan/shared/desktopIdentity";
...
interface StagePackageJson {
  ...
  readonly vulcanCommitHash: string;
  readonly vulcanLockfileSha256: string;
  readonly vulcanSourceTag: string | null;
  readonly vulcanWindowsPublisherSubject: string | null;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "VULCAN_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("VULCAN_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "VULCAN_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("VULCAN_DESKTOP_VERSION").pipe(Config.option),
  sourceCommit: Config.string("VULCAN_SOURCE_COMMIT").pipe(Config.option),
  sourceTag: Config.string("VULCAN_SOURCE_TAG").pipe(Config.option),
  lockfileSha256: Config.string("VULCAN_LOCKFILE_SHA256").pipe(Config.option),
  outputDir: Config.string("VULCAN_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.string("VULCAN_DESKTOP_SKIP_BUILD").pipe(Config.option),
  keepStage: Config.string("VULCAN_DESKTOP_KEEP_STAGE").pipe(Config.option),
  signed: Config.string("VULCAN_DESKTOP_SIGNED").pipe(Config.option),
  verbose: Config.string("VULCAN_DESKTOP_VERBOSE").pipe(Config.option),
  mockUpdates: Config.string("VULCAN_DESKTOP_MOCK_UPDATES").pipe(Config.option),
  mockUpdateServerPort: Config.string("VULCAN_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
});
...
export const resolveBuildOptions = Effect.fn("resolveBuildOptions")(function* (
  ...
  const envSkipBuild = yield* resolveBooleanEnv("VULCAN_DESKTOP_SKIP_BUILD", env.skipBuild);
  const envKeepStage = yield* resolveBooleanEnv("VULCAN_DESKTOP_KEEP_STAGE", env.keepStage);
  const envSigned = yield* resolveBooleanEnv("VULCAN_DESKTOP_SIGNED", env.signed);
  const envVerbose = yield* resolveBooleanEnv("VULCAN_DESKTOP_VERBOSE", env.verbose);
  const envMockUpdates = yield* resolveBooleanEnv("VULCAN_DESKTOP_MOCK_UPDATES", env.mockUpdates);
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.VULCAN_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const verifyStagedNodePty = Effect.fn("verifyStagedNodePty")(function* (
  ...
  yield* runCommand(
    ChildProcess.make({
      ...
      env: {
        ...process.env,
        VULCAN_NODE_PTY_SMOKE_REQUIRE_ROOT: stageAppDir,
      },
      ...commandOutputOptions(verbose),
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: VULCAN_PRODUCTION_BUNDLE_ID,
    productName,
    artifactName: "Vulcan-${version}-${arch}.${ext}",
    directories: {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `vulcan-desktop-${options.platform}-stage-`,
  });
  ...
  const resolvedBuildConfig = yield* createBuildConfig(
    options.platform,
    options.target,
    desktopPackageJson.productName ?? "Vulcan",
    options.signed,
    options.mockUpdates,
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    vulcanCommitHash: commitHash,
    vulcanLockfileSha256: resolvedLockfileSha256,
    vulcanSourceTag: options.sourceTag ?? null,
    vulcanWindowsPublisherSubject: resolvedBuildConfig.windowsPublisherSubject,
    private: true,
    description: "Vulcan desktop build",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: VULCAN_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: VULCAN_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: VULCAN_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: VULCAN_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  sourceCommit: Flag.string("source-commit").pipe(
    Flag.withDescription("Expected full source commit (env: VULCAN_SOURCE_COMMIT)."),
    Flag.optional,
  ),
  sourceTag: Flag.string("source-tag").pipe(
    Flag.withDescription("Exact source tag when building a release (env: VULCAN_SOURCE_TAG)."),
    Flag.optional,
  ),
  lockfileSha256: Flag.string("lockfile-sha256").pipe(
    Flag.withDescription("Expected bun.lock SHA-256 (env: VULCAN_LOCKFILE_SHA256)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: VULCAN_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: VULCAN_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: VULCAN_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: VULCAN_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: VULCAN_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: VULCAN_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.string("mock-update-server-port").pipe(
    Flag.withDescription("Mock update server port (env: VULCAN_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Vulcan."),
