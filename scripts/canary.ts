// FILE: canary.ts
// Purpose: Maintains and launches an isolated, frozen Vulcan Canary checkout.
...
export function resolveCanaryPaths(
  ...
  const home = Path.resolve(
    env.VULCAN_CANARY_HOME?.trim() || Path.join(homeDirectory, ".vulcan-canary"),
  );
  const cacheBase = env.XDG_CACHE_HOME?.trim() || Path.join(homeDirectory, ".cache");
  const source = Path.resolve(
    env.VULCAN_CANARY_SOURCE?.trim() || Path.join(cacheBase, "vulcan-canary", "source"),
  );
  return {
    home,
    source,
...
function assertManagedSourceIsClean(paths: CanaryPaths): void {
  ...
  if (status.length > 0) {
    throw new Error(
      `Vulcan Canary's managed source has tracked local changes. Refusing to overwrite ${paths.source}.`,
    );
  }
}
...
function startCanary(paths: CanaryPaths): void {
  ...
  if (existingPid !== null && isRunning(existingPid)) {
    console.log(`Vulcan Canary is already running (pid ${String(existingPid)}).`);
    return;
  }
  ...
  if (
    ...
    throw new Error("Vulcan Canary is not built. Run `bun run canary:setup` first.");
  }
  FS.mkdirSync(paths.home, { recursive: true });
  const env = { ...process.env };
  delete env.VITE_DEV_SERVER_URL;
  delete env.ELECTRON_RENDERER_PORT;
  delete env.VULCAN_AUTH_TOKEN;
  Object.assign(env, {
    VULCAN_DESKTOP_FLAVOR: "canary",
    VULCAN_DISABLE_AUTO_UPDATE: "1",
    VULCAN_HOME: paths.home,
    VULCAN_COMMIT_HASH: commit,
  });
  ...
  try {
    ...
    if (child.pid === undefined) {
      throw new Error("Vulcan Canary failed to return a process id.");
    }
    child.unref();
    FS.writeFileSync(paths.pid, `${String(child.pid)}\n`, { mode: 0o600 });
    console.log(`Started Vulcan Canary at ${commit.slice(0, 12)} (pid ${String(child.pid)}).`);
...
function rollbackCanary(paths: CanaryPaths): void {
  ...
  if (state?.previousCommit === null || state?.previousCommit === undefined) {
    throw new Error("Vulcan Canary has no previous successful commit to restore.");
  }
  assertManagedSourceIsClean(paths);
  stopCanary(paths);
...
function printStatus(paths: CanaryPaths): void {
  ...
  console.log(`Vulcan Canary: ${running ? `running (pid ${String(pid)})` : "stopped"}`);
