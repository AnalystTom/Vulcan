...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Vulcan.app"), [
      "Vulcan.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Vulcan.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Vulcan.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Vulcan.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Vulcan.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Vulcan.app/Contents/Info.plist",
        "Vulcan.app/Contents/Info.plist",
        "Vulcan.app/Contents/MacOS/Vulcan",
      ]),
      "Vulcan.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Vulcan-0.1.5-arm64.dmg",
        "Vulcan-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Vulcan-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Vulcan-0.1.5-arm64.zip", "Vulcan-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Vulcan-0.1.5-arm64.dmg"]),
...
files:
  - url: Vulcan-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Vulcan-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Vulcan-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Vulcan-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Vulcan-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Vulcan-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Vulcan-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Vulcan-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Vulcan-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Vulcan-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
