...
function createAssets(): string {
  const root = mkdtempSync(join(tmpdir(), "vulcan-artifact-provenance-test-"));
  temporaryRoots.push(root);
  writeFileSync(join(root, "Vulcan-1.2.3-x64.AppImage"), "app-image-bytes");
  writeFileSync(join(root, "latest-linux.yml"), "version: 1.2.3\n");
  return root;
}

function createWindowsAssets(): string {
  const root = mkdtempSync(join(tmpdir(), "vulcan-windows-provenance-test-"));
  temporaryRoots.push(root);
  writeFileSync(join(root, "Vulcan-1.2.3-x64.exe"), "unsigned-windows-bytes");
...
describe("release artifact provenance", () => {
  it("hashes the exact collected Linux assets into a deterministic manifest", async () => {
    ...
    expect(result.manifest.artifacts.map((artifact) => artifact.fileName)).toEqual([
      "latest-linux.yml",
      "Vulcan-1.2.3-x64.AppImage",
    ]);
    expect(
      result.manifest.artifacts.find(
        (artifact) => artifact.fileName === "Vulcan-1.2.3-x64.AppImage",
      )?.sha256,
