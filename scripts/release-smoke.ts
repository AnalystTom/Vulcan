...
files:
  - url: Vulcan-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
path: Vulcan-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: Vulcan-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
path: Vulcan-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function verifyCanonicalIdentity(): void {
  ...
  if (serverPackage.name !== "@vulcan/cli") {
    throw new Error(`Expected CLI package @vulcan/cli, got ${serverPackage.name ?? "<missing>"}.`);
  }
  const expectedBinaries = {
    vulcan: "dist/index.mjs",
    "vulcan-restore-migration-backup": "dist/restoreMigrationBackup.mjs",
  };
  if (JSON.stringify(serverPackage.bin ?? {}) !== JSON.stringify(expectedBinaries)) {
    throw new Error(
      "Expected the CLI to expose only the Vulcan entry point and migration recovery binary.",
    );
  }
  if (VULCAN_PRODUCTION_BUNDLE_ID !== "com.vulcanapp.vulcan") {
    throw new Error(`Unexpected production bundle ID: ${VULCAN_PRODUCTION_BUNDLE_ID}.`);
  }
  if (VULCAN_DESKTOP_UPDATE_CHANNEL !== "vulcan") {
    throw new Error(`Unexpected desktop update channel: ${VULCAN_DESKTOP_UPDATE_CHANNEL}.`);
  }
  ...
  if (
    ...
    throw new Error("Expected stable clean Vulcan releases to publish on GitHub Latest.");
  }
}
...
function verifyReleaseWorkflowSafety(): void {
  ...
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.VULCAN_PUBLISH_CLI == '1'",
    "Expected CLI publication to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.VULCAN_FINALIZE_RELEASE == '1'",
    "Expected release finalization to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "VULCAN_PUBLISH_RELEASE: ${{ needs.preflight.outputs.publish_release }}",
  ...
  assertContains(
    desktopBuildConfig,
    "__VULCAN_WINDOWS_UPDATER_PUBLISHER__",
...
const tempRoot = mkdtempSync(join(tmpdir(), "vulcan-release-smoke-"));
...
try {
  ...
  assertContains(
    mergedManifest,
    "Vulcan-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "Vulcan-9.9.9-smoke.0-x64.zip",
