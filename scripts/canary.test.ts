...
describe("canary tooling", () => {
  it("keeps managed source and Canary data separate from Stable", () => {
    expect(resolveCanaryPaths({}, "/Users/tester")).toEqual({
      home: "/Users/tester/.vulcan-canary",
      source: "/Users/tester/.cache/vulcan-canary/source",
      state: "/Users/tester/.vulcan-canary/canary-state.json",
      pid: "/Users/tester/.vulcan-canary/canary.pid",
      log: "/Users/tester/.vulcan-canary/canary.log",
    });
  });
  ...
  it("supports explicit path overrides", () => {
    expect(
      resolveCanaryPaths(
        {
          VULCAN_CANARY_HOME: "/tmp/canary-data",
          VULCAN_CANARY_SOURCE: "/tmp/canary-source",
        },
        "/Users/tester",
      ),
    ).toEqual({
  ...
  it("tracks main by default and accepts a stacked PR ref", () => {
    ...
    expect(parseCanaryArgs(["setup", "--ref", "codex/vulcan-canary"])).toEqual({
      command: "setup",
      ref: "codex/vulcan-canary",
    });
  });

  it("checks out the managed source during clone so the cleanliness guard starts clean", () => {
    expect(canaryCloneArgs("git@example.com:vulcan.git", "/tmp/canary-source")).toEqual([
      "clone",
      "--",
      "git@example.com:vulcan.git",
      "/tmp/canary-source",
    ]);
  });
  ...
  it("keeps updating the selected stacked ref until explicitly moved to main", () => {
    ...
    expect(resolveCanaryRef(parseCanaryArgs(["update"]), "codex/vulcan-canary")).toBe(
      "codex/vulcan-canary",
    );
