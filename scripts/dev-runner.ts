...
export const DEFAULT_VULCAN_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".vulcan"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@vulcan/contracts",
    "--filter=@vulcan/web",
    "--filter=@vulcan/cli",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=@vulcan/cli"],
  "dev:web": ["run", "dev", "--filter=@vulcan/web"],
  "dev:desktop": ["run", "dev", "--filter=@vulcan/desktop", "--filter=@vulcan/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("VULCAN_PORT_OFFSET"),
  devInstance: optionalStringConfig("VULCAN_DEV_INSTANCE"),
});
const HomeConfig = optionalStringConfig("VULCAN_HOME");
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid VULCAN_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `VULCAN_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric VULCAN_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed VULCAN_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_VULCAN_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly vulcanHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(vulcanHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      VULCAN_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://${formattedClientHost}:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      VULCAN_HOME: resolvedBaseDir,
      VULCAN_HOST: serverHost,
    };
    ...
    if (authToken !== undefined) {
      output.VULCAN_AUTH_TOKEN = authToken;
    } else {
      delete output.VULCAN_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.VULCAN_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.VULCAN_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.VULCAN_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.VULCAN_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.VULCAN_MODE = "web";
      delete output.VULCAN_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read VULCAN_PORT_OFFSET/VULCAN_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.VULCAN_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.VULCAN_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  vulcanHome: Flag.string("home-dir").pipe(
  ...
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to VULCAN_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("VULCAN_AUTH_TOKEN")),
  ),
  noBrowser: optionalBooleanFlag("no-browser", {
    description: "Disable browser auto-open (equivalent to VULCAN_NO_BROWSER).",
  ...
  autoBootstrapProjectFromCwd: optionalBooleanFlag("auto-bootstrap-project-from-cwd", {
    description:
      "Enable project auto-bootstrap (equivalent to VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
  }),
  logWebSocketEvents: optionalBooleanFlag("log-websocket-events", {
    description: "Enable WebSocket event logging (equivalent to VULCAN_LOG_WS_EVENTS).",
    aliases: ["log-ws-events"],
  }),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to VULCAN_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("VULCAN_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to VULCAN_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("VULCAN_PORT")),
  ),
