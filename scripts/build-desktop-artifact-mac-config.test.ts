...
describe("createDesktopPlatformBuildConfig", () => {
  it("adds explicit microphone entitlements to macOS builds", () => {
    ...
    assert.equal(MAC_APPSNAP_HELPER_BUNDLE_PATH, "Contents/Helpers/vulcan-appsnap-helper");
    assert.deepStrictEqual(mac.binaries, ["Contents/Helpers/vulcan-appsnap-helper"]);
    assert.equal(mac.x64ArchFiles, "Contents/Helpers/vulcan-appsnap-helper");
    assert.equal(
      MAC_APPSNAP_HELPER_STAGE_PATH,
      "apps/desktop/native/appsnap/build/vulcan-appsnap-helper",
    );
    ...
    assert.deepStrictEqual(config.extraFiles, [
      {
        from: "apps/desktop/native/appsnap/build/vulcan-appsnap-helper",
        to: "Helpers/vulcan-appsnap-helper",
      },
    ]);
  ...
  it("leaves non-macOS platform configs unchanged", () => {
    ...
    const win = createDesktopPlatformBuildConfig({
      platform: "win",
      target: "nsis",
      windowsAzureSignOptions: { publisherName: "Vulcan" },
    });
    ...
    assert.deepStrictEqual(win.win, {
      target: ["nsis"],
      icon: "icon.ico",
      publisherName: "Vulcan",
      azureSignOptions: { publisherName: "Vulcan" },
    });
  });
