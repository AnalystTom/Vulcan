...
function prepareWindowsLaunch(assetsDirectory: string, extractionRoot: string): LaunchCommand {
  ...
  const executables = findFiles(applicationRoot, (candidate) =>
    /[/\\]Vulcan\.exe$/i.test(candidate),
  );
  if (executables.length !== 1) {
    throw new Error(`Expected one extracted Vulcan.exe, found ${executables.length}.`);
  }
...
export function createPackagedDesktopSmokeEnvironment(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    VULCAN_HOME: join(root, "vulcan-home"),
  ...
  if (options.platform === "mac") {
    const userDataPath = join(env.HOME!, "Library", "Application Support", "vulcan");
...
export async function verifyPackagedDesktopStartup(
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), `vulcan-packaged-smoke-${options.platform}-`));
  ...
  try {
    ...
    const logPath = join(env.VULCAN_HOME!, "userdata", "logs", "desktop-main.log");
