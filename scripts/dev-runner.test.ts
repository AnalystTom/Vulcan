...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  it("allows every generated runtime setting through Turbo", () => {
    ...
    for (const name of [
      ...
      "VULCAN_AUTH_TOKEN",
      "VULCAN_PUBLIC_URL",
      "VULCAN_ALLOW_INSECURE_REMOTE",
      "VULCAN_HOST",
      "VULCAN_LOG_WS_EVENTS",
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit VULCAN_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "VULCAN_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid VULCAN_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("boolean precedence", () => {
    ...
    it.effect("rejects invalid boolean environment values", () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          readDevRunnerBooleanEnvironment({ VULCAN_LOG_WS_EVENTS: "sometimes" }),
        );
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("marks an inherited terminal PATH as already hydrated", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VULCAN_PATH_HYDRATED, "1");
        assert.match(env.PATH ?? "", /\/opt\/homebrew\/bin/);
      }),
    );

    it.effect("defaults VULCAN_HOME to ~/.vulcan when not provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.VULCAN_HOME, resolve(homedir(), ".vulcan"));
        assert.equal(env.VULCAN_HOST, "127.0.0.1");
    ...
    it.effect("normalizes bracketed IPv6 hosts for listen and client URL syntax", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VULCAN_HOST, "::1");
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VULCAN_HOME, resolve("/tmp/custom-vulcan"));
        assert.equal(env.VULCAN_PORT, "4222");
        assert.equal(env.VULCAN_NO_BROWSER, "1");
        assert.equal(env.VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.VULCAN_LOG_WS_EVENTS, "1");
        assert.equal(env.VULCAN_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            VULCAN_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          vulcanHome: undefined,
        ...
        assert.equal(env.VULCAN_MODE, "web");
        assert.equal(env.VULCAN_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VULCAN_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom vulcanHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.VULCAN_HOME, resolve("/tmp/my-vulcan"));
        assert.equal(env.VULCAN_HOME, resolve("/tmp/my-vulcan"));
        assert.equal(env.VULCAN_HOME, resolve("/tmp/my-vulcan"));
      }),
    );
