# Verify: run Vulcan locally for runtime verification

How to launch an isolated Vulcan instance (server + web) to observe UI changes, without touching `~/.vulcan` or the default dev ports.

## Launch

```bash
# 1. Server (from the directory you want as the workspace/project cwd):
VULCAN_HOME=<scratch>/vulcan-home \
VULCAN_PORT=3899 VULCAN_MODE=web VULCAN_NO_BROWSER=1 \
