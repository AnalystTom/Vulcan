...
export const Bot = Schema.Struct({
  ...
  isolationMode: BotIsolationMode.pipe(
    Schema.withDecodingDefault(() => DEFAULT_BOT_ISOLATION_MODE),
  ),
  autonomy: BotAutonomyConfig.pipe(Schema.withDecodingDefault(() => DEFAULT_BOT_AUTONOMY)),
  capabilityGrants: Schema.Array(BotCapability).pipe(
...
export const BotCommsChannel = Schema.Struct({
  ...
  lastMessagePreview: Schema.NullOr(
    Schema.String.check(Schema.isMaxLength(BOT_COMMS_PREVIEW_MAX_CHARS)),
  ),
...
export const BotAuditListInput = Schema.Struct({
  botId: BotId,
  limit: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 500 })).pipe(
