  ...
  it("matches expected fields independent of order and extra certificate fields", () => {
    expect(
      matchesDistinguishedName(
        ...
        "C=US, O=Acme Tools, CN=Vulcan, SERIALNUMBER=1234",
      ),
    ).toBe(true);
  });

  it("rejects incomplete pins and mismatched signer fields", () => {
    expect(matchesDistinguishedName("CN=Vulcan", "CN=Vulcan, O=Acme Tools")).toBe(false);
    expect(matchesDistinguishedName("CN=Vulcan, O=Acme Tools", "CN=Vulcan, O=Other Tools")).toBe(
      false,
    );
  });
});
