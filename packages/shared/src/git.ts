 ...
 */
export const WORKTREE_BRANCH_PREFIX = "vulcan";
...
const VULCAN_BRANCH_FALLBACK = "update";
...
export function buildVulcanBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || VULCAN_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueVulcanBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildVulcanBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
