...
import type { ServerLocalServerProcess } from "@vulcan/contracts";
...
describe("localServerPrimaryLabel", () => {
  it("prefers the live page title when one was resolved", () => {
    expect(localServerPrimaryLabel(makeServer({ pageTitle: "Vulcan", displayName: "Vite" }))).toBe(
      "Vulcan",
    );
  });
...
describe("localServerFolderLabel", () => {
  it("returns the final segment of a POSIX cwd", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/vulcan-website" }))).toBe(
      "vulcan-website",
    );
  });

  it("ignores a trailing separator", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/vulcan/" }))).toBe(
      "vulcan",
    );
  });
