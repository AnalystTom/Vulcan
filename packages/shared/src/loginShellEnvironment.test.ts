...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "vulcan-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Vulcan home.
    const env = { VULCAN_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Vulcan home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { VULCAN_HOME: "/tmp/vulcan-home" } })).toBe(
      Path.join("/tmp/vulcan-home", "cache", "login-shell-environment.json"),
    );
    expect(loginShellEnvironmentCachePath({ env: {}, homeDirectory: "/users/test" })).toBe(
      Path.join("/users/test", ".vulcan", "cache", "login-shell-environment.json"),
    );
  });
});
