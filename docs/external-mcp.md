...
Vulcan can expose a small, user-approved MCP tool surface to another local app such as Codex or
Claude. This integration is separate from the internal `/mcp` endpoint injected into provider
sessions.

## Guided setup

1. Start Vulcan and open **Settings → Integrations**.
...
4. For Codex, Claude Code, or another agentic MCP client, copy the generated setup prompt into the
   ...
   stdio configuration, and verifies the connection with `vulcan_overview`. For Claude Desktop or a
   client that cannot run the setup prompt, complete pairing in Vulcan and use the copy-ready JSON
   configuration instead. Vulcan uses the exact executable and data directory of the running
   installation; no global `vulcan` command, project ID, model slug, request ID, or credential
   handling is required from the user.
5. Vulcan moves from **Waiting for pairing** to **Paired** after the local credential exchange, then
...
```sh
/absolute/path/to/runtime /absolute/path/to/vulcan-server mcp serve --integration mcp_int_REDACTED --home-dir "$HOME/.vulcan"
```

The Codex copy action generates:

```sh
codex mcp add vulcan [--env ELECTRON_RUN_AS_NODE=1] -- /absolute/runtime /absolute/server mcp serve --integration mcp_int_REDACTED --home-dir "$HOME/.vulcan"
```

The Claude Code copy action generates a user-scoped configuration:

```sh
claude mcp add --scope user vulcan [-e ELECTRON_RUN_AS_NODE=1] -- /absolute/runtime /absolute/server mcp serve --integration mcp_int_REDACTED --home-dir "$HOME/.vulcan"
```
...
args = ["/absolute/path/to/vulcan-server", "mcp", "serve", "--integration", "mcp_int_REDACTED", "--home-dir", "/absolute/path/to/vulcan-data"]
...
```json
{
  "mcpServers": {
    "vulcan": {
      "command": "/absolute/path/to/runtime",
      "args": [
        "/absolute/path/to/vulcan-server",
        "mcp",
        "serve",
        "--integration",
        "mcp_int_REDACTED",
        "--home-dir",
        "/absolute/path/to/vulcan-data"
      ],
...
Vulcan always includes its actual data directory in generated setup, so multiple installations do not
...
`<Vulcan home>/mcp/credentials/<integration-id>.json`. Vulcan creates its parent directory with mode `0700`
and the file with mode `0600` on POSIX systems. On Windows the file remains under the current user
profile, but Windows does not provide the same POSIX mode guarantee; protect the account and Vulcan
...
- `vulcan_overview` — orient in one call with allowed projects, paths and activity, provider
  availability, granted scopes, safe defaults, limits, and suggested next steps.
- `vulcan_capabilities` — provider/model construction and safety limits for an allowed project.
- `vulcan_list_allowed_projects` — only projects selected by the user.
- `vulcan_create_task` — one task per stable `requestId`.
- `vulcan_wait_for_task` — wait for an authorized task without changing it.
- `vulcan_read_task` — read tasks created by the integration. Reading other tasks requires the
...
- `/mcp/external` is available only while Vulcan itself is loopback-only. Configuring remote or
  published access disables the external endpoint instead of exposing it remotely.
- External credentials have the fixed `vulcan.external-mcp` audience. They are opaque, expiring,
...
The stdio bridge re-reads Vulcan's private runtime-state file on each request and requires the
