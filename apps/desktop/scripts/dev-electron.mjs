...
function cleanupStaleDevApps() {
  ...
  const commandPattern = `^${executable}[[:space:]]+--vulcan-dev-root=${devRoot}([[:space:]]|$)`;
  spawnSync("pkill", ["-f", "--", commandPattern], { stdio: "ignore" });
}

function listStaleComputerUsePids() {
  // Only macOS exposes a verifiable Vulcan (Dev) executable path for these
  ...
  return candidatePids.filter((pid) => {
    ...
    if (!/Vulcan \(Dev\)\.app\/Contents\/MacOS\/Electron/.test(command)) {
      return false;
    }
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Vulcan (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const pids = listPidsByExactProcessName("Vulcan").filter((pid) =>
    readProcessCommand(pid).startsWith("/Applications/Vulcan.app/Contents/MacOS/Vulcan"),
  );
  if (pids.length === 0) {
    return;
  }
  ...
  console.error(
    "[desktop-dev] Vulcan is still running. Close it before testing voice in Vulcan (Dev), or you may be looking at the wrong app/runtime.",
  );
  console.error(`[desktop-dev] Running Vulcan process IDs: ${pids.join(", ")}`);
}
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--vulcan-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
