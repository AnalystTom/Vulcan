...
import type { BrowserToolName, ThreadId } from "@vulcan/contracts";
...
const PIPE_DIR = "vulcan-browser-host";
const PIPE_NAME_PREFIX = "vulcan-browser-host";

export const VULCAN_BROWSER_HOST_PIPE_ENV = "VULCAN_BROWSER_HOST_PIPE_PATH";
export const VULCAN_BROWSER_HOST_CAPABILITY_ENV = "VULCAN_BROWSER_HOST_CAPABILITY";
export const VULCAN_BROWSER_HOST_CAPABILITY_FD_ENV = "VULCAN_BROWSER_HOST_CAPABILITY_FD";
/** @deprecated Read/written only while old backend builds are still supported. */
export const VULCAN_BROWSER_USE_PIPE_ENV = "VULCAN_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserHostPipePath(
  ...
  const configured =
    env[VULCAN_BROWSER_HOST_PIPE_ENV]?.trim() || env[VULCAN_BROWSER_USE_PIPE_ENV]?.trim();
...
export const VULCAN_BROWSER_HOST_PIPE_PATH = resolveConfiguredBrowserHostPipePath();
/** @deprecated Compatibility alias for old packaged backend builds. */
export const VULCAN_BROWSER_USE_PIPE_PATH = VULCAN_BROWSER_HOST_PIPE_PATH;
...
export function resolveBrowserHostPipeBackendEnv(
  ...
  delete backendEnv[VULCAN_BROWSER_HOST_PIPE_ENV];
  delete backendEnv[VULCAN_BROWSER_USE_PIPE_ENV];
  delete backendEnv[VULCAN_BROWSER_HOST_CAPABILITY_ENV];
  delete backendEnv[VULCAN_BROWSER_HOST_CAPABILITY_FD_ENV];
  const pipePath = activePipePath?.trim();
  if (pipePath && Number.isInteger(capabilityFd) && (capabilityFd ?? 0) >= 3) {
    backendEnv[VULCAN_BROWSER_HOST_PIPE_ENV] = pipePath;
    backendEnv[VULCAN_BROWSER_USE_PIPE_ENV] = pipePath;
    backendEnv[VULCAN_BROWSER_HOST_CAPABILITY_FD_ENV] = String(capabilityFd);
  }
  return backendEnv;
}
...
export class BrowserHostPipeServer {
  ...
  constructor(
    ...
    options: BrowserHostPipeServerOptions | string = VULCAN_BROWSER_HOST_PIPE_PATH,
  ) {
    const normalized = typeof options === "string" ? { pipePath: options } : options;
    this.platform = normalized.platform ?? process.platform;
    this.pipePath = normalized.pipePath ?? VULCAN_BROWSER_HOST_PIPE_PATH;
  ...
  private getInfo(client: PipeClient, params: unknown): unknown {
    ...
    return {
      name: "Vulcan Browser Host",
      version: "1.0.0",
      type: "vulcan-browser-host",
      metadata: {
        sessionId,
        protocolVersion: 1,
