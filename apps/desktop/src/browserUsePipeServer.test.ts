...
import {
  ...
  VULCAN_BROWSER_HOST_CAPABILITY_FD_ENV,
  VULCAN_BROWSER_HOST_PIPE_ENV,
  VULCAN_BROWSER_USE_PIPE_ENV,
  resolveBrowserHostPipeBackendEnv,
  resolveConfiguredBrowserHostPipePath,
  resolveDefaultBrowserHostPipePath,
} from "./browserUsePipeServer";

const TEST_CAPABILITY = "vulcan-browser-host-test-capability-0123456789";
...
async function withPipeServer(
  ...
  const directory = await mkdtemp(join(tmpdir(), "vulcan-browser-host-test-"));
...
describe("canonical browser host pipe resolution", () => {
  it("creates a private unguessable Unix socket path", () => {
    ...
    expect(dirname(first)).toBe(`/tmp/vulcan-browser-host${uidSuffix}`);
  ...
  it("creates an unguessable per-process Windows named pipe", () => {
    ...
    expect(pipePath).toMatch(/^\\\\\.\\pipe\\vulcan-browser-host-456-[0-9a-f-]{36}$/);
  ...
  it("prefers the canonical env and accepts the legacy env as fallback", () => {
    expect(
      resolveConfiguredBrowserHostPipePath(
        {
          [VULCAN_BROWSER_HOST_PIPE_ENV]: "/canonical.sock",
          [VULCAN_BROWSER_USE_PIPE_ENV]: "/legacy.sock",
        },
        "darwin",
      ),
    ).toBe("/canonical.sock");
    expect(
      resolveConfiguredBrowserHostPipePath(
        {
          [VULCAN_BROWSER_USE_PIPE_ENV]: String.raw`\\.\pipe\legacy`,
        },
        "win32",
      ),
  ...
  it("publishes both env names only while a listener is active", () => {
    const inherited = {
      KEEP_ME: "yes",
      [VULCAN_BROWSER_HOST_PIPE_ENV]: "/stale-canonical.sock",
      [VULCAN_BROWSER_USE_PIPE_ENV]: "/stale-legacy.sock",
    };
    ...
    expect(resolveBrowserHostPipeBackendEnv(inherited, "/active.sock", 3)).toEqual({
      KEEP_ME: "yes",
      [VULCAN_BROWSER_HOST_PIPE_ENV]: "/active.sock",
      [VULCAN_BROWSER_USE_PIPE_ENV]: "/active.sock",
      [VULCAN_BROWSER_HOST_CAPABILITY_FD_ENV]: "3",
    });
  });

  it("rejects a non-private configured parent without changing its permissions", async () => {
    const directory = await mkdtemp(join(tmpdir(), "vulcan-browser-host-public-test-"));
...
describe("canonical browser host RPC", () => {
  it("accepts every client in a maximum-size gateway batch", async () => {
    const directory = await mkdtemp(join(tmpdir(), "vulcan-browser-host-capacity-test-"));
    const pipePath = join(directory, "browser.sock");
    const server = new BrowserHostPipeServer({} as never, {
      pipePath,
      capability: TEST_CAPABILITY,
