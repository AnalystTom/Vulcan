...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "vulcan-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Vulcan profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "vulcan-dev" })).toBe(
      "/Users/tester/Library/Application Support/vulcan-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "vulcan" })).toBe(
      "/Users/tester/Library/Application Support/vulcan",
    );
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "vulcan-canary" }),
    ).toBe("/Users/tester/Library/Application Support/vulcan-canary");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "vulcan");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "vulcan-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "vulcan-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Vulcan profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Vulcan database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "vulcan");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".vulcan-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "vulcan");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "vulcan-profile-seed.json"), "{");
