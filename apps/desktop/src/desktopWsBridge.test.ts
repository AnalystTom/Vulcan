...
describe("desktopWsBridge", () => {
  ...
  it("reads only the canonical Vulcan desktop URL environment value", () => {
    expect(
      resolveDesktopWsUrlFromEnv({
        VULCAN_DESKTOP_WS_URL: "ws://127.0.0.1:6000/?token=vulcan",
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
    ).toBe("ws://127.0.0.1:6000/?token=vulcan");
    expect(
      resolveDesktopWsUrlFromEnv({
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
