...
import type { ContextMenuItem } from "@vulcan/contracts";
import { isKeyboardShortcutsHelpChord } from "@vulcan/shared/browserShortcuts";
import { getMacTrafficLightPosition } from "@vulcan/shared/desktopChrome";
import {
  VULCAN_DESKTOP_UPDATE_CHANNEL,
  resolveVulcanDesktopFlavor,
  vulcanDesktopIdentity,
} from "@vulcan/shared/desktopIdentity";
import { NetService } from "@vulcan/shared/Net";
import { applyShellEnvironmentHydrationMarker } from "@vulcan/shared/shell";
import { RotatingFileSink } from "@vulcan/shared/logging";
import { ensureStaticSnapshot, findAsarArchivePath } from "@vulcan/shared/staticSnapshot";
...
import {
  ...
  resolveVulcanStorageSnapshotPath,
...
// `BASE_DIR` prefers VULCAN_HOME, which the Windows registry read hydrates whenever the
...
const desktopFlavor = resolveVulcanDesktopFlavor({
  isDevelopment,
  requestedFlavor: process.env.VULCAN_DESKTOP_FLAVOR,
});
const desktopIdentity = vulcanDesktopIdentity(desktopFlavor);
const BASE_DIR =
  process.env.VULCAN_HOME?.trim() ||
...
const BACKEND_MAX_OLD_SPACE_ENV_KEYS = ["VULCAN_BACKEND_MAX_OLD_SPACE_MB"] as const;
...
const VULCAN_BROWSER_LABEL = "Vulcan browser";
const browserPerfLoggingEnabled = process.env.VULCAN_BROWSER_PERF === "1";
...
function startBrowserPerformanceLogging(): void {
  ...
  browserPerfInterval = setInterval(() => {
    ...
    console.info(`[${VULCAN_BROWSER_LABEL} perf]`, {
      ...snapshot.counters,
...
async function ensureBrowserHostPipeServer(): Promise<void> {
  if (browserHostPipeServer || !VULCAN_BROWSER_HOST_PIPE_PATH) {
    return;
  }
...
async function reserveBackendEndpoint(reason: string): Promise<void> {
  ...
  process.env.VULCAN_DESKTOP_WS_URL = backendWsUrl;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { vulcanCommitHash?: unknown };
    return normalizeCommitHash(parsed.vulcanCommitHash);
  } catch {
    return null;
  }
}

declare const __VULCAN_WINDOWS_UPDATER_PUBLISHER__: string;
...
function resolveEmbeddedWindowsPublisherSubjects(): string[] {
  ...
  const subject = __VULCAN_WINDOWS_UPDATER_PUBLISHER__.trim();
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.VULCAN_COMMIT_HASH);
  if (envCommitHash) {
...
async function handleDesktopMigrationRecovery(): Promise<DesktopMigrationRecoveryOutcome> {
  ...
  const outcome = await recoverDesktopMigrationIfRequired({
    ...
    choose: async ({ previousFailure }) => {
      // The user is here because Vulcan cannot open its database, so the
      ...
      if (canInstallUpdate) {
        choices.push({
          label: "Update Vulcan and restart",
          detail: "install the newest Vulcan release, which may already contain the fix",
      ...
      if (releaseUrl !== null) {
        choices.push({
          ...
          detail: `${canInstallUpdate ? "download that release" : "download the latest Vulcan release"} in a browser`,
      ...
      const result = await dialog.showMessageBox({
        ...
        title:
          previousFailure === null
            ? "Vulcan needs to recover its database"
            : restoreFailed
              ? "Migration recovery failed"
              : "Vulcan could not update itself",
        message:
          previousFailure === null
            ? "Vulcan stopped a database migration before it could finish safely."
            : restoreFailed
              ? "The saved database backup could not be restored."
              : "The newest Vulcan release could not be installed.",
...
// vulcan:// protocol here and, via VULCAN_STATIC_DIR, for the backend's HTTP static
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Vulcan failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
...
function hasConfiguredUpdateFeed(): boolean {
  return readAppUpdateYml() !== null || Boolean(process.env.VULCAN_DESKTOP_MOCK_UPDATES);
}
...
function resolveAutoUpdateDisabledReason(): string | null {
  return getAutoUpdateDisabledReason({
    ...
    disabledByEnv:
      desktopIdentity.usesScriptedUpdates || process.env.VULCAN_DISABLE_AUTO_UPDATE === "1",
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Vulcan ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
    ...
    void dialog.showMessageBox({
      type: "info",
      title: "Update found",
      message: "Vulcan is preparing the update in the background.",
      buttons: ["OK"],
    });
...
function resolveNotificationIconPath(): string | null {
  ...
  if (process.platform === "win32") {
    return resolveResourcePath("vulcan.png") ?? resolveIconPath("ico");
  }
  return resolveResourcePath("vulcan.png") ?? resolveIconPath("png");
}

function resolveAppSnapHelperPath(): string {
  if (app.isPackaged) {
    return Path.resolve(process.resourcesPath, "..", "Helpers", "vulcan-appsnap-helper");
  }
  return Path.resolve(__dirname, "..", ".electron-runtime", "appsnap", "vulcan-appsnap-helper");
}
 ...
 * package.json. We override it to a clean lowercase Vulcan name.
 */
...
function repairBrowserProfileBeforeElectronReady(userDataPath: string): void {
  ...
  if (browserProfileRepair.status === "repaired") {
    console.info("[desktop] Completed Vulcan browser profile bridge repair", {
    ...
    console.warn("[desktop] Failed to complete Vulcan browser profile bridge repair", {
      sourcePath: browserProfileRepair.sourcePath,
      targetPath: browserProfileRepair.targetPath,
...
function configureAppIdentity(): void {
  ...
  app.setAboutPanelOptions({
    ...
    copyright: `© ${new Date().getFullYear()} Vulcan contributors`,
  });
...
function restartAfterStartupBundleSwap(error: BundleChangedDuringStartupError): void {
  ...
  void dialog
    .showMessageBox({
      type: "warning",
      title: "Vulcan needs to restart",
      message: "Vulcan changed while it was opening.",
      detail:
        "The current process cannot safely read the replaced application bundle. Restart Vulcan to finish opening with one consistent version.",
      buttons: ["Restart Vulcan"],
      defaultId: 0,
    })
    .catch(() => undefined)
    .then(() => {
...
// every archive read in this process — the vulcan:// protocol, the backend's static
...
function startBundleSwapWatcher(): void {
  ...
  bundleSwapPollTimer = setInterval(() => {
    ...
    void dialog
      .showMessageBox({
        type: "warning",
        title: "Vulcan was replaced on disk",
        message: "The installed Vulcan app changed while it was running.",
        detail:
...
function processInstallMarkerOnStartup(): void {
  ...
  const message = `Vulcan restarted, but update ${marker.toVersion} was not installed. Try again.`;
  setUpdateState(
    reduceDesktopUpdateStateOnInstallRestartFailure(
      updateState,
      marker.toVersion,
...
async function installLatestUpdateForMigrationRecovery(): Promise<string | null> {
  ...
  if (updateState.status === "up-to-date") {
    return `Vulcan ${app.getVersion()} is already the newest release, so updating cannot repair this database.`;
  }
  if (updateState.status !== "downloaded") {
...
function configureAutoUpdater(): void {
  ...
  // default feed while Vulcan versions advance independently.
  autoUpdater.channel = VULCAN_DESKTOP_UPDATE_CHANNEL;
...
// Builds process-local Node args so provider/tool children do not inherit Vulcan's heap guard.
...
function backendEnv(): NodeJS.ProcessEnv {
  ...
  const env: NodeJS.ProcessEnv = {
    ...resolveBrowserHostPipeBackendEnv(
      process.env,
      browserHostPipeServer ? VULCAN_BROWSER_HOST_PIPE_PATH : null,
    ...
    // vulcan:// protocol serves, so both surfaces survive app.asar being replaced.
    ...(servedStaticRoot?.snapshotted ? { VULCAN_STATIC_DIR: servedStaticRoot.dir } : {}),
    VULCAN_MODE: "desktop",
    VULCAN_NO_BROWSER: "1",
    VULCAN_PORT: String(backendPort),
    VULCAN_HOME: BASE_DIR,
    VULCAN_AUTH_TOKEN: backendAuthToken,
    VULCAN_DESKTOP_SHUTDOWN_TOKEN: DESKTOP_BACKEND_SHUTDOWN_TOKEN,
  };
...
function backendFailureDialogDetail(reason: string): string {
  ...
  return [
    cause,
    "Vulcan paused automatic restarts so a failing backend can't keep respawning in the background.",
...
function presentBackendStartupGiveUp(reason: string): void {
  ...
  const task = (async () => {
    for (;;) {
      const result = await dialog.showMessageBox({
        type: "error",
        title: "Vulcan's backend didn't start",
        message: `Vulcan's backend failed to start ${BACKEND_MAX_CONSECUTIVE_START_FAILURES} times in a row.`,
        detail,
...
function handleBackendStartupBlock(block: BackendStartupBlock): void {
  ...
  const task = (async () => {
    if (block.kind === "migration-recovery-required") {
      const result = await dialog.showMessageBox({
        type: "warning",
        title: "Vulcan needs to recover its database",
        message: "A database migration did not finish safely.",
        detail:
          "Restart Vulcan to open the verified backup recovery flow. Provider and chat processes will remain stopped until recovery completes.",
    ...
    const processDetail =
      block.ownerPid === null
        ? "Another Vulcan server is already using this database."
        : `Another Vulcan server (process ${block.ownerPid}) is already using this database.`;
    const result = await dialog.showMessageBox({
      type: "warning",
      title: "Vulcan is already running elsewhere",
      message: "Your local Vulcan data is in use by another process.",
      detail: `${processDetail}\n\nStop the other Vulcan app or development server, then try again. Your data has not been changed.`,
...
function startBackend(trigger: BackendStartTrigger = "lifecycle"): void {
  ...
  const child = ChildProcess.spawn(process.execPath, [...backendNodeArgs(), backendEntry], {
    ...
    env: {
      ...
      VULCAN_SERVER_ENTRY: backendEntry,
    },
...
function getTitleBarOptions(): BrowserWindowConstructorOptions {
  ...
  return {
    ...
    // Derived from the shared chat-surface header geometry (@vulcan/shared/desktopChrome)
...
function presentRendererCrashRecovery(
  ...
  const message =
    response.cause === "reload-budget-exhausted"
      ? `Vulcan's window crashed ${response.crashes} times in a row.`
      : "Vulcan's window stopped unexpectedly.";
  const detail = [
    `The window's renderer process exited (${reason}).`,
    response.cause === "reload-budget-exhausted"
      ? "Vulcan paused automatic reloads so a repeating crash can't keep reloading in the background."
      : "This exit reason repeats on reload, so Vulcan did not retry automatically.",
...
function configureMediaPermissions(): void {
  ...
  const permissionTargets = [
    ...
    },
    {
      ...
      // microphone grant used by Vulcan's own voice-composer renderer.
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    console.warn("[Vulcan browser] Failed to start browser host pipe", error);
  }
  startBackend();
