...
describe("resolveLaunchVersionRecordPath", () => {
  it("places the record file inside the userData directory", () => {
    expect(resolveLaunchVersionRecordPath("/home/me/AppData/Vulcan")).toBe(
      "/home/me/AppData/Vulcan/last-launch-version.json",
    );
  });
});
...
describe("resolveMacAppBundlePath", () => {
  it("resolves the .app bundle from the Electron executable on macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Vulcan.app/Contents/MacOS/Vulcan", "darwin"),
    ).toBe("/Applications/Vulcan.app");
  });

  it("returns null off macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Vulcan.app/Contents/MacOS/Vulcan", "linux"),
    ).toBeNull();
  });

  it("returns null when the executable is not inside a .app bundle", () => {
    expect(resolveMacAppBundlePath("/usr/local/bin/vulcan", "darwin")).toBeNull();
  });
});
