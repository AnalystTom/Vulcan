...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    VULCAN_CLAUDE_KEEPALIVE: "0",
    VULCAN_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    VULCAN_HOME: input.homeDir,
    VULCAN_HOST: "127.0.0.1",
    VULCAN_MODE: "desktop",
    VULCAN_NO_BROWSER: "1",
    VULCAN_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("vulcan-windows-shutdown-");
