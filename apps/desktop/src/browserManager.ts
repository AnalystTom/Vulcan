...
import { isBrowserCopyLinkChord } from "@vulcan/shared/browserShortcuts";
import {
...
export class DesktopBrowserManager {
  ...
  private scheduleWindowOpenTab(input: {
    ...
    // runtimes. Only the first decision may create a canonical Vulcan tab.
    if (
  ...
  attachWebview(input: BrowserAttachWebviewInput, hostWebContentsId: number): ThreadBrowserState {
    ...
    if (
      ...
      throw new Error("The browser webview does not belong to this Vulcan window and partition.");
    }
