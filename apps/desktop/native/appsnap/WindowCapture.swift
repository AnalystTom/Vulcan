    ...
    if application.bundleIdentifier == excludedBundleIdentifier {
        return .failure(
            AppSnapFailure(
                ...
                message: "Vulcan cannot capture its own window."
            )
        )
    }
...
final class OneFrameWindowCapture: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let outputQueue = DispatchQueue(label: "dev.vulcan.appsnap.stream-output")
...
final class AppSnapCaptureCoordinator {
    ...
    private let queue = DispatchQueue(label: "dev.vulcan.appsnap.capture")
