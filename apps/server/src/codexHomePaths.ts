...
export const VULCAN_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveVulcanCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.VULCAN_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".vulcan", "runtime");
  return path.join(overlayRoot, VULCAN_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Vulcan keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveVulcanCodexHomeOverlayPath(env, source);
