...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "vulcan-terminal-"));
  ...
  it("keeps a running terminal alive when open reattaches with different cwd or env", async () => {
    ...
    const snapshot = await manager.open(
      openInput({ cwd: logsDir, env: { VULCAN_TERMINAL_TEST: "changed" } }),
    );
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.VULCAN_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          VULCAN_PROJECT_ROOT: "/repo",
          VULCAN_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.VULCAN_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.VULCAN_WORKTREE_PATH).toBe("/repo/worktree-a");
