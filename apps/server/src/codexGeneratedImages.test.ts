...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousVulcanHome === undefined) delete process.env.VULCAN_HOME;
    else process.env.VULCAN_HOME = previousVulcanHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.VULCAN_HOME = "/vulcan-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/vulcan-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.vulcan/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.vulcan/runtime/codex-home-overlay";
