...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("vulcan-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("vulcan-codex-home-");
  ...
  it("allows images written to the VULCAN_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Vulcan overlay
    // directory (see resolveVulcanCodexHomeOverlayPath). Generated images therefore
    // live under <VULCAN_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const vulcanHome = path.join(fakeRoot, "vulcan", "runtime");
    ...
    process.env.VULCAN_HOME = vulcanHome;
    try {
      const result = await resolveAllowedLocalPreviewFile({
        requestedPath: imagePath,
        cwd: null,
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("vulcan-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/vulcan-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "vulcan-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("vulcan-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("vulcan-image-tmp-root-");
