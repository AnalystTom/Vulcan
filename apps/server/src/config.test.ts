...
const originalVulcanStaticDir = process.env.VULCAN_STATIC_DIR;

function makeTempDir(prefix = "vulcan-config-test-"): string {
...
afterEach(() => {
  ...
  tempDirs.clear();
  if (originalVulcanStaticDir === undefined) {
    delete process.env.VULCAN_STATIC_DIR;
  } else {
    process.env.VULCAN_STATIC_DIR = originalVulcanStaticDir;
  }
});

describe("resolveStaticDir", () => {
  it("uses the desktop static snapshot exposed through the Vulcan environment", async () => {
    const snapshotDir = makeTempDir("vulcan-static-snapshot-test-");
    fs.writeFileSync(path.join(snapshotDir, "index.html"), "<main>Vulcan</main>");
    process.env.VULCAN_STATIC_DIR = snapshotDir;
...
describe("resolveDefaultChatWorkspaceRoot", () => {
  it("places the managed chat workspace under Documents/Vulcan on macOS and Linux", () => {
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Vulcan");
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Vulcan");
  });
  ...
  it("uses Windows separators when deriving the managed chat workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Vulcan");
  });
  ...
  it("defaults to the current process platform when no platform is supplied", () => {
    ...
    try {
      expect(resolveDefaultChatWorkspaceRoot({ homeDir: "C:\\Users\\tester" })).toBe(
        "C:\\Users\\tester\\Documents\\Vulcan",
      );
    } finally {
...
describe("resolveDefaultStudioWorkspaceRoot", () => {
  it("places the Studio workspace under Documents/Vulcan/Studio on macOS and Linux", () => {
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Vulcan/Studio");
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Vulcan/Studio");
  });
  ...
  it("uses Windows separators when deriving the Studio workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Vulcan\\Studio");
  });
});
...
describe("resolveCanonicalWorkspaceRoots", () => {
  it("canonicalizes a symlinked home directory to match project row realpaths", async () => {
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedHomeDir, "Documents", "Vulcan"));
    expect(result.studioWorkspaceRoot).toBe(
      path.join(expectedHomeDir, "Documents", "Vulcan", "Studio"),
    );
  });
  ...
  it("canonicalizes the nearest existing ancestor when the workspace root itself does not exist yet", async () => {
    ...
    // Vulcan/ nor Vulcan/Studio exist yet underneath it.
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedDocuments, "Vulcan"));
    expect(result.studioWorkspaceRoot).toBe(path.join(expectedDocuments, "Vulcan", "Studio"));
