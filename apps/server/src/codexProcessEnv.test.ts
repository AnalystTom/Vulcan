...
describe("linkOrCopyCodexOverlayEntry", () => {
  it("copies auth.json when symlink creation is unavailable", async () => {
    ...
    await linkOrCopyCodexOverlayEntry(
      {
        ...
        targetPath: "C:\\Users\\test\\.vulcan\\codex-home-overlay\\auth.json",
        type: "file",
      },
      { symlink, copyFile },
    );
    ...
    expect(symlink).toHaveBeenCalledWith(
      ...
      "C:\\Users\\test\\.vulcan\\codex-home-overlay\\auth.json",
      "file",
    );
      ...
      linkOrCopyCodexOverlayEntry(
        {
          ...
          targetPath: "C:\\Users\\test\\.vulcan\\codex-home-overlay\\sessions",
          type: "dir",
        },
...
describe("buildCodexProcessEnv", () => {
  it("replaces a user-defined Vulcan MCP table only inside the session overlay", async () => {
    const sourceHome = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-source-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-runtime-"));
    const sourceConfig = [
      'model = "gpt-5.5"',
      "",
      "[mcp_servers.vulcan]",
      ...
      "",
      "[mcp_servers.vulcan.headers]",
      'Authorization = "stale-inline-secret"',
      "",
      "[mcp_servers.vulcan.env]",
      'STALE_GATEWAY_TOKEN = "stale-inline-secret"',
      "",
      "[mcp_servers.vulcan-other]",
      'url = "http://127.0.0.1:2111/vulcan-other"',
      "",
    ...
    const managedConfig = [
      ...
      'bearer_token_env_var = "VULCAN_AGENT_GATEWAY_TOKEN"',
      "",
      "[shell_environment_policy]",
      'exclude = ["VULCAN_AGENT_GATEWAY_TOKEN"]',
    ].join("\n");
    ...
    try {
      ...
      if (!overlayHome) {
        throw new Error("Expected a Vulcan Codex home overlay.");
      }
      const overlayConfig = readFileSync(path.join(overlayHome, "config.toml"), "utf8");

      expect(overlayConfig.match(/^\[mcp_servers\.vulcan\]$/gm)).toHaveLength(1);
      expect(overlayConfig).toContain('url = "http://127.0.0.1:3773/mcp"');
      expect(overlayConfig).toContain('bearer_token_env_var = "VULCAN_AGENT_GATEWAY_TOKEN"');
      ...
      expect(overlayConfig).not.toContain("[mcp_servers.vulcan.headers]");
      expect(overlayConfig).not.toContain("[mcp_servers.vulcan.env]");
      ...
      expect(overlayConfig).toContain('exclude = ["VULCAN_AGENT_GATEWAY_TOKEN", "USER_SECRET"]');
