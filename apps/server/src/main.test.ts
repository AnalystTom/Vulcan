...
import { CliConfig, makeServerStartupLogData, vulcanCli, type CliConfigShape } from "./main";
...
let defaultVulcanHome = "";
const tempHomes = new Set<string>();

function makeTempHome(prefix = "vulcan-main-test-"): string {
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(vulcanCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            VULCAN_HOME: defaultVulcanHome,
            VULCAN_NO_BROWSER: "true",
            ...env,
          },
        }),
      ),
...
beforeEach(() => {
  vi.clearAllMocks();
  defaultVulcanHome = makeTempHome();
  resolvedConfig = null;
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("vulcan-main-flag-");
  ...
  it.effect("passes the root --home-dir flag to MCP subcommands", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("vulcan-main-mcp-flag-");
  ...
  it.effect("creates fresh local state directories with private permissions", () =>
    Effect.gen(function* () {
      ...
      const homeDir = makeTempHome("vulcan-main-private-fresh-");
  ...
  it.effect("repairs permissions for an upgraded local state directory", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return;
      const homeDir = makeTempHome("vulcan-main-private-upgrade-");
      const stateDir = path.join(homeDir, "userdata");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      const envHome = makeTempHome("vulcan-main-env-");

      yield* runCli([], {
        VULCAN_MODE: "desktop",
        VULCAN_PORT: "4999",
        VULCAN_HOST: "127.0.0.1",
        VULCAN_HOME: envHome,
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        VULCAN_NO_BROWSER: "true",
        VULCAN_AUTH_TOKEN: "env-token",
        VULCAN_DESKTOP_SHUTDOWN_TOKEN: "shutdown-token",
      });
  ...
  it.effect("consumes desktop shutdown authority before generic child launches", () =>
    Effect.gen(function* () {
      const canonicalKey = "VULCAN_DESKTOP_SHUTDOWN_TOKEN";
      const mixedCaseKey = "vUlCaN_dEsKtOp_ShUtDoWn_ToKeN";
      ...
      try {
        ...
        resolvedConfig = null;
        yield* runCli([], { VULCAN_DESKTOP_SHUTDOWN_TOKEN: injectedToken });
  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        VULCAN_AUTH_TOKEN: "browser-secret",
        VULCAN_DESKTOP_SHUTDOWN_TOKEN: "shutdown-secret",
      });
  ...
  it.effect("prefers --mode over VULCAN_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over VULCAN_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        VULCAN_NO_BROWSER: "false",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        ...
        ],
        {
          VULCAN_MODE: "desktop",
          VULCAN_NO_BROWSER: "true",
          VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
          VULCAN_LOG_PROVIDER_EVENTS: "true",
          VULCAN_LOG_WS_EVENTS: "true",
        },
      );
  ...
  it.effect("honors insecure remote opt-in from the environment when the CLI flag is absent", () =>
    Effect.gen(function* () {
      yield* runCli(["--host", "0.0.0.0", "--auth-token", "remote-secret"], {
        VULCAN_ALLOW_INSECURE_REMOTE: "true",
        VULCAN_NO_BROWSER: "true",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        [
          ...
          "remote-secret",
          "--public-url",
          "https://vulcan.example.test",
        ],
        { VULCAN_NO_BROWSER: "false" },
      );

      assert.equal(resolvedConfig?.publicUrl?.origin, "https://vulcan.example.test");
      assert.equal(openBrowser.mock.calls.length, 1);
      assert.match(
        openBrowser.mock.calls[0]?.[0] ?? "",
        /^https:\/\/vulcan\.example\.test\/pair#token=/,
      );
    }),
  );
  ...
  it.effect("supports the HTTPS public origin through environment configuration", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        VULCAN_HOST: "192.168.1.50",
        VULCAN_AUTH_TOKEN: "remote-secret",
        VULCAN_PUBLIC_URL: "https://vulcan.example.test",
      });
  ...
  it.effect("rejects non-root or non-HTTPS public URLs", () =>
    Effect.gen(function* () {
      for (const publicUrl of ["http://vulcan.example.test", "https://vulcan.example.test/app"]) {
        const error = yield* Effect.flip(
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Vulcan to non-loopback host 0\.0\.0\.0/);
    }),
  );
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        VULCAN_MODE: "desktop",
        VULCAN_LOG_PROVIDER_EVENTS: "true",
        VULCAN_LOG_WS_EVENTS: "false",
        VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        VULCAN_NO_BROWSER: "true",
      });
  ...
  it.effect("rejects invalid boolean environment values instead of treating them as absent", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        runCli([], {
          VULCAN_LOG_PROVIDER_EVENTS: "sometimes",
        }),
      );
