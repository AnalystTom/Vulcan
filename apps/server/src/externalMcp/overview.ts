import type { ExternalMcpCapability } from "@vulcan/contracts";
...
export function buildExternalMcpOverviewNextSteps(
  ...
  return [
    ...
    ...(capabilities.has("tasks:create") ? ["Create work with vulcan_create_task."] : []),
    ...(capabilities.has("tasks:wait") ? ["Follow permitted work with vulcan_wait_for_task."] : []),
    ...(capabilities.has("tasks:read")
      ? ["Read permitted task results with vulcan_read_task."]
      : []),
  ];
}
