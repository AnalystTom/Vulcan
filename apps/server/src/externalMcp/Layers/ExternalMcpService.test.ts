import { ProjectId } from "@vulcan/contracts";
...
const layer = ExternalMcpServiceLive.pipe(
  ...
  Layer.provide(Layer.succeed(ServerConfig, { baseDir: "/tmp/vulcan-test" } as never)),
);
    ...
    await run(
      Effect.gen(function* () {
        ...
        const secondPair = yield* service
          .pair(created.pairingCode, "vul_mcp_v1_different-secret")
          .pipe(Effect.exit);
  ...
  it("grants every current and future project with the all-projects scope", async () => {
    await run(
      Effect.gen(function* () {
        ...
        const paired = yield* service.pair(created.pairingCode, "vul_mcp_v1_all-scope-secret");
  ...
  it("surfaces repository audit completion failures to the gateway boundary", async () => {
    await run(
      Effect.gen(function* () {
        ...
        const paired = yield* service.pair(created.pairingCode, "vul_mcp_v1_audit-failure-secret");
        const client = yield* service.verifyCredential(paired.credential);
        const auditId = yield* service.beginAudit(client, {
          tool: "vulcan_list_allowed_projects",
        });
        yield* sql`
