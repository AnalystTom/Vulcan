...
import { MessageId, ProjectId, TurnId } from "@vulcan/contracts";
...
describe("external MCP gateway stdio flow", () => {
  it("pairs, filters tools, creates one safe task, waits, reads, and audits without prompt leakage", async () => {
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "vulcan-external-e2e-"));
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const paired = yield* service.pair(
          issued.pairingCode,
          "vul_mcp_v1_e2e-client-generated-secret",
        );
        ...
        stdin.write(
          `${JSON.stringify({
            ...
            id: 4,
            method: "tools/call",
            params: { name: "vulcan_read_task", arguments: { threadId } },
          })}\n`,
        );
        stdin.end();
        ...
        const interruptedWait = yield* gateway
          .handlePost({
            authorizationHeader: "Bearer vul_mcp_v1_e2e-client-generated-secret",
            body: {
              jsonrpc: "2.0",
              id: "interrupted-wait",
              method: "tools/call",
        ...
        const restrictedCredential = "vul_mcp_v1_restricted-audit-secret";
        ...
        const overview = yield* gateway.handlePost({
          ...
          body: {
            ...
            id: "overview",
            method: "tools/call",
            params: { name: "vulcan_overview", arguments: {} },
          },
        });
        ...
        expect(overviewPayload.nextSteps).toEqual([
          "Call vulcan_capabilities with a projectId to list the exact provider/model targets available to this integration.",
        ]);
        ...
        const successfulDespiteAuditFailure = yield* gateway.handlePost({
          ...
          body: {
            ...
            method: "tools/call",
            params: { name: "vulcan_list_allowed_projects", arguments: {} },
          },
        });
