...
function makeBaseDir() {
  const value = fs.mkdtempSync(path.join(os.tmpdir(), "vulcan-mcp-bridge-test-"));
...
describe("external MCP stdio bridge", () => {
  ...
  it("passes hostile Windows runtime paths outside the encoded PowerShell program", () => {
    ...
    expect(invocation.options.env.VULCAN_RUNTIME_ACL_TARGET).toBe(targetPath);
    ...
    expect(
      requestTimeoutForBody(
        JSON.stringify({
          ...
          id: 1,
          method: "tools/call",
          params: { name: "vulcan_wait_for_task", arguments: { threadId: "thread-1" } },
        }),
      ),
    ).toBe(35_000);
    expect(
      ...
      requestTimeoutForBody(
        JSON.stringify([
          ...
          },
          {
            ...
            id: 4,
            method: "tools/call",
            params: { name: "vulcan_wait_for_task", arguments: { threadId: "thread-2" } },
          },
        ]),
      ),
    ).toBe(65_000);
    expect(
      requestTimeoutForBody(
        JSON.stringify({
          ...
          id: 5,
          method: "tools/call",
          params: { name: "vulcan_create_task", arguments: {} },
        }),
      ),
    ).toBe(605_000);
  });
  ...
  it("fails clearly for missing and multiple running instances", () => {
    ...
    expect(() => discoverExternalMcpRuntime(baseDir)).toThrow(/No running Vulcan instance/);
    writeRuntime(baseDir, "userdata", 3773);
    writeRuntime(baseDir, "dev", 4773);
    expect(() => discoverExternalMcpRuntime(baseDir)).toThrow(/Multiple running Vulcan instances/);
  });
  ...
  it.skipIf(process.platform === "win32")("rejects a symlinked MCP secret directory", () => {
    ...
    expect(() =>
      writeExternalMcpClientCredential(baseDir, {
        ...
        name: "Symlink",
        credential: "vul_mcp_v1_must-not-escape",
        expiresAt: "2026-08-20T00:00:00.000Z" as never,
      }),
  ...
  it("selects one stored integration explicitly and rejects an ambiguous default", () => {
    ...
    for (const integrationId of ["integration-a", "integration-b"]) {
      writeExternalMcpClientCredential(baseDir, {
        integrationId,
        name: integrationId,
        credential: `vul_mcp_v1_${integrationId}`,
        expiresAt: "2026-08-20T00:00:00.000Z" as never,
      });
    }
  ...
  it("forwards a complete stdio MCP request and returns the JSON-RPC response", async () => {
    ...
    await serveExternalMcpStdio({
      ...
      fetchImpl: async (url, init) => {
        ...
        expect(new Headers(init?.headers).get("authorization")).toBe("Bearer vul_mcp_v1_stdio");
    ...
    await serveExternalMcpStdio({
      ...
      fetchImpl: async (url, init) => {
        ...
        if (urls.length === 1) {
          ...
          throw new TypeError("old Vulcan instance stopped");
        }
  ...
  it("reports a revoked credential without leaking it", async () => {
    ...
    writeExternalMcpClientCredential(baseDir, {
      ...
      name: "Claude",
      credential: "vul_mcp_v1_do-not-leak",
      expiresAt: "2026-08-20T00:00:00.000Z" as never,
    });
    const output: string[] = [];
    await serveExternalMcpStdio({
  ...
  it("does not answer failed notifications and preserves every request id in a failed batch", async () => {
    ...
    writeExternalMcpClientCredential(baseDir, {
      ...
      name: "Protocol",
      credential: "vul_mcp_v1_protocol-errors",
      expiresAt: "2026-08-20T00:00:00.000Z" as never,
    });
    const output: string[] = [];
    await serveExternalMcpStdio({
  ...
  it("allows a fast request to complete while a wait request is still pending", async () => {
    ...
    writeExternalMcpClientCredential(baseDir, {
      ...
      name: "Concurrent",
      credential: "vul_mcp_v1_concurrent",
      expiresAt: "2026-08-20T00:00:00.000Z" as never,
    });
    const output: string[] = [];
    await serveExternalMcpStdio({
      baseDir,
      stdin: Readable.from([
        `${JSON.stringify({ jsonrpc: "2.0", id: "slow", method: "tools/call", params: { name: "vulcan_wait_for_task", arguments: { timeoutMs: 100 } } })}\n`,
  ...
  it("aborts an active HTTP request when the MCP client cancels its request id", async () => {
    ...
    writeExternalMcpClientCredential(baseDir, {
      ...
      name: "Cancellation",
      credential: "vul_mcp_v1_cancellation",
      expiresAt: "2026-08-20T00:00:00.000Z" as never,
    });
    ...
    stdin.write(
      `${JSON.stringify({ jsonrpc: "2.0", id: "slow", method: "tools/call", params: { name: "vulcan_wait_for_task", arguments: { threadId: "thread-1" } } })}\n`,
    );
  ...
  it("cancels one batched request without aborting its siblings", async () => {
    ...
    writeExternalMcpClientCredential(baseDir, {
      ...
      credential: "vul_mcp_v1_batch-cancellation",
      expiresAt: "2026-08-20T00:00:00.000Z" as never,
    });
    const stdin = new PassThrough();
    const output: string[] = [];
  ...
  it("reuses the locally persisted client secret after a lost pairing response", async () => {
    ...
    expect(fs.readFileSync(paired.storePath, "utf8")).not.toContain("vul_pair_v1_retry");
  });
});
