...
const DESKTOP_SHUTDOWN_TOKEN_ENV_KEY = "VULCAN_DESKTOP_SHUTDOWN_TOKEN";
...
interface CliInput {
  ...
  readonly vulcanHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("VULCAN_MODE").pipe(
    Config.option,
    Config.map(
  ...
  port: Config.port("VULCAN_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("VULCAN_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  vulcanHome: Config.string("VULCAN_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  publicUrl: Config.url("VULCAN_PUBLIC_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  allowInsecureRemote: optionalBooleanEnvironmentConfig("VULCAN_ALLOW_INSECURE_REMOTE"),
  noBrowser: optionalBooleanEnvironmentConfig("VULCAN_NO_BROWSER"),
  authToken: Config.string("VULCAN_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  desktopShutdownToken: Config.string("VULCAN_DESKTOP_SHUTDOWN_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: optionalBooleanEnvironmentConfig(
    "VULCAN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
  ),
  logProviderEvents: optionalBooleanEnvironmentConfig("VULCAN_LOG_PROVIDER_EVENTS"),
  logWebSocketEvents: optionalBooleanEnvironmentConfig("VULCAN_LOG_WS_EVENTS"),
});
    ...
    Effect.gen(function* () {
      ...
      if (configuredPublicUrl && publicUrl === undefined) {
        return yield* new StartupError({
          message:
            "VULCAN_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://vulcan.example.com).",
        });
      }
      ...
      const configuredHome = Option.getOrUndefined(input.vulcanHome) ?? env.vulcanHome;
      ...
      yield* Effect.try({
        try: () => preparePrivateServerPaths(derivedPaths),
          ...
          new StartupError({ message: "Failed to secure Vulcan's local state directory", cause }),
      });
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Vulcan running", makeServerStartupLogData(config));
      ...
      if (config.allowInsecureRemote && !config.publicUrl) {
        yield* Effect.logWarning(
          "INSECURE REMOTE ACCESS ENABLED: credentials and session traffic are unencrypted",
          {
            ...
            hint: "Use only on a trusted LAN. Configure VULCAN_PUBLIC_URL behind HTTPS for protected remote access.",
          },
        );
      }
...
const vulcanHomeFlag = Flag.string("home-dir").pipe(
  Flag.withDescription("Base directory for all Vulcan data (equivalent to VULCAN_HOME)."),
  Flag.optional,
);
...
const publicUrlFlag = Flag.string("public-url").pipe(
  ...
  Flag.withDescription(
    "HTTPS public root origin provided by a TLS-terminating reverse proxy (equivalent to VULCAN_PUBLIC_URL).",
  ),
  Flag.optional,
);
const allowInsecureRemoteFlag = optionalBooleanFlag("allow-insecure-remote", {
  description:
    "Explicitly allow unencrypted authenticated remote access on a trusted LAN (equivalent to VULCAN_ALLOW_INSECURE_REMOTE).",
});
...
const logProviderEventsFlag = optionalBooleanFlag("log-provider-events", {
  description:
    "Emit native/canonical provider NDJSON logs for debugging (equivalent to VULCAN_LOG_PROVIDER_EVENTS).",
});
const logWebSocketEventsFlag = optionalBooleanFlag("log-websocket-events", {
  description:
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to VULCAN_LOG_WS_EVENTS).",
...
// Base `vulcan` command defined before the MCP subcommands so they can yield
// its parsed input (notably `--home-dir` / `vulcanHome`) via Effect's command
...
const baseServerCommand = Command.make("vulcan", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  vulcanHome: vulcanHomeFlag,
  devUrl: devUrlFlag,
...
}).pipe(Command.withDescription("Run the Vulcan server."));
...
const mcpServeCommand = Command.make(
  ...
  ({ integration }) =>
    Effect.gen(function* () {
      ...
      const baseDir = resolveExternalMcpBaseDir(Option.getOrUndefined(parent.vulcanHome));
  ...
  Command.withDescription(
    "Serve the paired Vulcan external MCP integration over stdio for Codex, Claude, and other MCP clients.",
  ),
);
...
const mcpPairCommand = Command.make(
  "pair",
  {
    code: Flag.string("code").pipe(
      Flag.withDescription("Short-lived pairing code issued by Vulcan Settings."),
    ),
  },
  ({ code }) =>
    Effect.gen(function* () {
      ...
      process.stdout.write(
        `Paired Vulcan external MCP integration "${paired.paired.name}".\nCredential stored privately at ${paired.storePath}.\nConfigure the MCP client command as: ${externalMcpShellCommand(externalMcpLauncher(["mcp", "serve", "--integration", paired.paired.integrationId, "--home-dir", baseDir]))}\n`,
      );
      if (process.platform === "win32") {
        process.stdout.write(
          "Windows note: Vulcan stores this credential under your user profile, but Windows does not expose POSIX 0600 permission checks. Protect the profile and its Vulcan data directory.\n",
        );
      }
    }),
).pipe(Command.withDescription("Pair this CLI with a user-approved Vulcan MCP integration."));

const mcpCommand = Command.make("mcp").pipe(
  Command.withDescription("Manage Vulcan's loopback external MCP bridge."),
...
export const vulcanCli = serverCommand;
