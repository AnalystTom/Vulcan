...
import type { AutomationDefinition, AutomationRun } from "@vulcan/contracts";
import { automationContinuesThread, automationOwnsItsThread } from "@vulcan/shared/automationMode";
...
// vulcan_cancel_automation covers standalone runs, whose per-run thread owns nothing else.
const SELF_CANCEL_INSTRUCTION =
  "You may call vulcan_cancel_automation on this automation when it is no longer needed.";
...
function reportingInstructions(mode: AutomationDefinition["mode"]): string {
  ...
  if (automationContinuesThread(mode)) {
    return [
      "Before finishing, call vulcan_report_automation_result.",
      'Use decision "silent" when nothing needs the user\'s attention; otherwise use "notify".',
      SELF_CANCEL_INSTRUCTION,
    ].join(" ");
  }
  return [
    "Before finishing, call vulcan_report_automation_result with a concise title and summary.",
...
export function buildAutomationRunEnvelope(input: {
  ...
  return [
    ...
    'Memory (persistent across runs — replace it via vulcan_update_automation_memory {"memory": "..."} before finishing):',
