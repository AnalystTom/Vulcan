...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Vulcan refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/vulcan\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/vulcan/", "refs/historical/");
