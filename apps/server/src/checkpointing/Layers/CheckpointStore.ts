  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "vulcan-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_EMAIL: "vulcan@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Vulcan",
              GIT_COMMITTER_EMAIL: "vulcan@users.noreply.github.com",
            };
            ...
            const message = `Vulcan checkpoint ref=${input.checkpointRef}`;
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "vulcan-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
