...
import { isTemporaryWorktreeBranch, WORKTREE_BRANCH_PREFIX } from "@vulcan/shared/git";
import { claudeSelectionRequiresRestart } from "@vulcan/shared/model";
...
} from "@vulcan/shared/providerDeliveryBlock";
import { buildStalePendingRequestFailureDetail } from "@vulcan/shared/threadSummary";
import { resolveThreadWorkspaceState } from "@vulcan/shared/threadEnvironment";
...
function buildGeneratedWorktreeBranchName(raw: string): string {
  ...
  const withoutPrefix = normalized.replace(/^vulcan\//, "");
...
>()("vulcan/orchestration/Layers/ProviderCommandReactorConfig") {}
    ...
    if (input.reviewTarget !== undefined) {
      ...
      const replayWithTranscriptBootstrap = (
        ...
        Effect.gen(function* () {
          ...
          // dead cursor and replay once with Vulcan transcript context.
          yield* clearStaleProviderResumeState({
            threadId: input.threadId,
            cause,
          ...
          if (!startupRecoveryNotifiedThreads.has(blocker.threadId)) {
            ...
            if (skippedPromptCount > 0) {
              ...
              yield* appendProviderFailureActivity({
                ...
                detail: `Vulcan recovered an earlier provider failure, but ${skippedPromptCount} ${noun} skipped while the thread was blocked. Resend ${skippedPromptCount === 1 ? "it" : "them"} to continue.`,
                turnId: null,
                createdAt,
              });
