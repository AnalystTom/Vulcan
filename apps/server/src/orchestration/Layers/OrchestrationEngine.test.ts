...
const TestServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "vulcan-orchestration-engine-test-",
});
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/vulcan/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
        files: [],
  ...
  it("rejects duplicate Studio workspace containers", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/vulcan-studio",
        defaultModelSelection: null,
        createdAt,
      }),
    );
  ...
  it("rejects Studio and regular projects claiming each other's workspace root", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/vulcan-cross-kind-studio",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    await system.run(
      engine.dispatch({
        ...
        kind: "project",
        title: "App",
        workspaceRoot: "/tmp/vulcan-cross-kind-app",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    ...
    await expect(
      system.run(
        engine.dispatch({
          ...
          projectId: asProjectId("project-cross-kind-app"),
          kind: "studio",
          workspaceRoot: "/tmp/vulcan-cross-kind-pinned-studio",
        }),
      ),
  ...
  it("rejects moving a Studio container onto another Studio workspace root", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/vulcan-studio-source",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/vulcan-studio-target",
        defaultModelSelection: null,
        createdAt,
      }),
    );
