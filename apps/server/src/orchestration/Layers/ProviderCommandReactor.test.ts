...
import { PROVIDER_DELIVERY_BLOCK_SUMMARY } from "@vulcan/shared/providerDeliveryBlock";
import {
  Duration,
  Effect,
  Exit,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "vulcan-reactor-"));
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await waitFor(async () => {
      const thread = await readHarnessThread(harness);
      return (
        thread?.branch === "vulcan/app-startup-crash" &&
        thread.associatedWorktreeBranch === "vulcan/app-startup-crash" &&
        thread.associatedWorktreeRef === "vulcan/app-startup-crash"
      );
    });

    const thread = await readHarnessThread(harness);
    expect(thread).toMatchObject({
      branch: "vulcan/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "vulcan/app-startup-crash",
      associatedWorktreeRef: "vulcan/app-startup-crash",
    });
  });
  ...
  it("uses the configured Git-writing model when the chat provider cannot generate names", async () => {
    ...
    expect(harness.renameBranch.mock.calls[0]?.[0]).toMatchObject({
      oldBranch: "vulcan/cb661f0d",
      newBranch: "vulcan/provider-startup-timeouts",
    });

    await waitFor(
      async () => (await readHarnessThread(harness))?.branch === "vulcan/provider-startup-timeouts",
    );
  });
