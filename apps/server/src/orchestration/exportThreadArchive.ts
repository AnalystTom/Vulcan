...
// Purpose: Build a ZIP archive that exports a single Vulcan thread so a user
...
// The export-eligibility guard lives in @vulcan/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@vulcan/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `vulcan-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
