...
// Purpose: Compute Profile-page stats from Vulcan's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@vulcan/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Vulcan user message, plus the
...
>()("vulcan/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Vulcan prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Vulcan's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "vulcan";
