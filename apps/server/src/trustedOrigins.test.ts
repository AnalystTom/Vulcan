...
describe("trustedOrigins", () => {
  ...
  it("trusts only the HTTPS public origin for browser traffic in proxy-backed remote mode", () => {
    ...
    expect(
      isTrustedAppOrigin({
        ...
        requestOrigin: "http://vulcan.example.test",
        config: remoteConfig,
      }),
    ).toBe(true);
    expect(
  ...
  it("normalizes desktop origins with trailing slashes", () => {
    expect(normalizeCorsOrigin("vulcan://app/")).toBe("vulcan://app");
    expect(normalizeCorsOrigin("vulcan-canary://app/")).toBe("vulcan-canary://app");
  });
