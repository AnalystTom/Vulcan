...
describe("migration backup recovery CLI", () => {
  it("ships the recovery command from the bundled server package", async () => {
    ...
    expect(packageJson.bin?.["vulcan-restore-migration-backup"]).toBe(
      "dist/restoreMigrationBackup.mjs",
    );
  });

  it("rejects relative database paths and warns operators to stop Vulcan", async () => {
    ...
    expect(capture.errors.join("\n")).toContain("vulcan-restore-migration-backup");
    expect(capture.warnings.join("\n")).toContain("Stop every Vulcan process");
