...
// Purpose: Builds provider child environments without Vulcan control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("VULCAN_FAKE_") || key.startsWith("VULCAN_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedVulcanKeys?: ReadonlyArray<string>;
  ...
  const allowedVulcanKeys = new Set(input.inheritedVulcanKeys ?? []);
  ...
  for (const [key, value] of Object.entries(baseEnv)) {
    if (
      key.startsWith("VULCAN_") &&
      !allowedVulcanKeys.has(key) &&
