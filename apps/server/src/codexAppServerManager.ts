...
import { prewarmChatGptVoiceTranscriptionConnection } from "@vulcan/shared/chatGptVoiceTranscription";
import { getModelSelectionBooleanOptionValue, normalizeModelSlug } from "@vulcan/shared/model";
import { decodeSubagentReceiverThreadIds } from "@vulcan/shared/subagents";
...
import { VULCAN_GATEWAY_HARNESS_POLICY } from "./agentGateway/harnessPolicy.ts";
import {
...
Prefer Vulcan's built-in browser for browser work. It may continue in the background without changing the user's active chat. In code mode, call its MCP methods directly inside \`functions.exec\` with the exact \`tools.mcp__vulcan__browser_*\` prefix (for example, \`await tools.mcp__vulcan__browser_open({ url })\` and \`await tools.mcp__vulcan__browser_snapshot({})\`). The available suffixes are ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
...
// Maps Vulcan's simple runtime toggle to Codex thread-level permission overrides.
...
// Vulcan re-sends turn-level Codex permission overrides, so keep "always allow"
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly vulcanSkillsDir: string | undefined;
  ...
  constructor(
    ...
    options?: {
      readonly vulcanSkillsDir?: string;
    ...
    this.vulcanSkillsDir = options?.vulcanSkillsDir;
  ...
  private async buildSessionProcessEnv(
    ...
    if (gatewayBearerToken) {
      env[VULCAN_AGENT_GATEWAY_TOKEN_ENV] = gatewayBearerToken;
    }
    return env;
  }

  // Registers `~/.vulcan/skills` as a codex skill root so portable skills are
  ...
  private async registerVulcanSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.vulcanSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.vulcanSkillsDir],
      });
    } catch (error) {
      // Older codex builds (< extra-roots support) keep working; Vulcan-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      await this.registerVulcanSkillsRoot(context);
  ...
  private async handleServerRequest(
    ...
    if (isUserInputRequest) {
      ...
      const detail = "Codex asked a question Vulcan could not render, so it was declined.";
