...
import {
  ...
  VULCAN_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS,
...
describe("Codex Vulcan harness policy", () => {
  it("keeps the same host policy exactly once in default and plan instructions", () => {
    for (const instructions of [
      CODEX_DEFAULT_MODE_DEVELOPER_INSTRUCTIONS,
      CODEX_PLAN_MODE_DEVELOPER_INSTRUCTIONS,
    ]) {
      expect(instructions).toContain(VULCAN_HARNESS_POLICY_MARKER);
      expect(instructions.split(VULCAN_HARNESS_POLICY_MARKER)).toHaveLength(2);
      expect(instructions).toContain("Vulcan is the host and harness");
      expect(instructions).toContain("one exact vulcan_create_threads plan");
      expect(instructions).toContain("tools.mcp__vulcan__browser_open");
  ...
  it("resolves the gateway endpoint when each session environment is built", async () => {
    const homePath = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-gateway-endpoint-"));
    const previousVulcanHome = process.env.VULCAN_HOME;
    process.env.VULCAN_HOME = path.join(homePath, "vulcan-home");
    ...
    try {
      ...
      if (previousVulcanHome === undefined) {
        delete process.env.VULCAN_HOME;
      } else {
        process.env.VULCAN_HOME = previousVulcanHome;
      }
...
describe("codex CLI version gate", () => {
  it("memoizes the version probe per binary and shares concurrent probes", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-version-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("VULCAN_HOME", path.join(dir, "runtime"));
  ...
  it("does not reuse a general-version verdict for the stricter Auto floor", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-version-auto-floor-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("VULCAN_HOME", path.join(dir, "runtime"));
  ...
  it("fails closed for Auto when the Codex CLI version cannot be parsed", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-version-auto-unknown-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("VULCAN_HOME", path.join(dir, "runtime"));
  ...
  it("re-probes when the binary behind an unchanged path is replaced", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-version-swap-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("VULCAN_HOME", path.join(dir, "runtime"));
    ...
    try {
      ...
      await expect(
        assertSupportedCodexCliVersion({ binaryPath, cwd: dir, homePath }),
      ).rejects.toThrow(/too old for Vulcan/);
    } finally {
      reset();
      vi.unstubAllEnvs();
      rmSync(dir, { recursive: true, force: true });
  ...
  it("re-probes when a PATH-resolved codex is replaced behind the same bare name", async () => {
    ...
    const dir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-version-path-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("VULCAN_HOME", path.join(dir, "runtime"));
  ...
  it("rejects an unsupported codex version without caching the failure", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-version-old-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("VULCAN_HOME", path.join(dir, "runtime"));
...
describe("buildCodexProcessEnv", () => {
  it("hydrates the active custom provider env_key from the effective CODEX_HOME", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-env-"));
    try {
      writeFileSync(
  ...
  it("keeps the private desktop browser host out of the Codex process", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-private-host-"));
    ...
    try {
      const env = await buildCodexProcessEnv({
        env: {
          ...
          VULCAN_HOME: tempDir,
          VULCAN_BROWSER_HOST_PIPE_PATH: "/tmp/vulcan-browser-host.sock",
          VULCAN_BROWSER_USE_PIPE_PATH: "/tmp/legacy-browser-use.sock",
          VULCAN_BROWSER_HOST_CAPABILITY: "desktop-capability",
          VULCAN_BROWSER_HOST_CAPABILITY_FD: "3",
          NODE_REPL_SANDBOX_ALLOWED_UNIX_SOCKETS: "/tmp/existing.sock",
        },
        platform: "darwin",
      });

      expect(env.VULCAN_BROWSER_HOST_PIPE_PATH).toBeUndefined();
      expect(env.VULCAN_BROWSER_USE_PIPE_PATH).toBeUndefined();
      expect(env.VULCAN_BROWSER_HOST_CAPABILITY).toBeUndefined();
      expect(env.VULCAN_BROWSER_HOST_CAPABILITY_FD).toBeUndefined();
  ...
  it("applies durable section suppressions inside Vulcan's Codex overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "vulcan-runtime-home-"));
    try {
      writeFileSync(
        path.join(tempDir, "config.toml"),
        [
          ...
          "enabled = true",
          "",
          ...VULCAN_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS.flatMap((header) => [
            header,
            "enabled = true",
            "",
          ]),
      ...
      writeFileSync(
        path.join(overlayHome, "vulcan-config-suppressions-v1.json"),
        `${JSON.stringify({
          version: 1,
      ...
      for (const header of VULCAN_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS) {
        expect(readFileSync(path.join(codexHome, "config.toml"), "utf8")).toContain(
          `${header}\nenabled = false`,
        );
  ...
  it("seeds markerless suppressions for conflicting local browser plugins", async () => {
    ...
    try {
      ...
      const suppressionMarker = JSON.parse(
        readFileSync(path.join(overlayHome, "vulcan-config-suppressions-v1.json"), "utf8"),
  ...
  it("repairs stale real files in Vulcan's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "vulcan-runtime-home-"));
    try {
  ...
  it("repairs stale auth.json files in Vulcan's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "vulcan-runtime-home-"));
    try {
  ...
  it("preserves real generated image directories in Vulcan's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "vulcan-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "vulcan-runtime-home-"));
    try {
      writeFileSync(path.join(tempDir, "config.toml"), 'model = "gpt-5.5"', "utf8");
...
describe("handleStdoutLine", () => {
  ...
  it("ignores multiline and standalone JSON leaked from command output", () => {
    ...
    for (const line of ["{", "[", '{"scripts": {', "{}", "[]", '{"name":"vulcan"}']) {
      handleStdoutLine(context, line);
    }
...
describe("startSession", () => {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}vulcan-codex-workspaces${path.sep}thread-1`);
  });

  it("reports a missing project working directory instead of a missing Codex CLI", () => {
    const missingCwd = path.join(os.tmpdir(), `vulcan-missing-cwd-${randomUUID()}`, "old-project");
  ...
  it("accepts an existing project working directory", () => {
    const cwd = mkdtempSync(path.join(os.tmpdir(), "vulcan-existing-cwd-"));
    try {
  ...
  it("fails session start with missing-cwd guidance instead of missing Codex CLI", async () => {
    ...
    const missingCwd = path.join(
      os.tmpdir(),
      `vulcan-missing-session-cwd-${randomUUID()}`,
      "old-project",
    );
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Vulcan. Upgrade to v0.37.0 or newer and restart Vulcan.",
        );
      });
