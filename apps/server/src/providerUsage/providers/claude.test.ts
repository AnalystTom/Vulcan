...
import { outboundHttp } from "@vulcan/shared/outboundHttp";
import {
...
function makeClaudeHome(creds: Record<string, unknown>) {
  const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "vulcan-claude-usage-"));
...
function makeClaudeConfigDir(creds: Record<string, unknown>) {
  const configDir = mkdtempSync(nodePath.join(os.tmpdir(), "vulcan-claude-config-"));
...
describe("claudeUsageFetcher", () => {
  it("prefers the current macOS account before the service-only keychain fallback", async () => {
    const homeDir = mkdtempSync(nodePath.join(os.tmpdir(), "vulcan-claude-keychain-"));
    tempDirs.push(homeDir);
