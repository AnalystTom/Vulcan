...
import type { BrowserToolName, ProviderKind, ThreadId } from "@vulcan/contracts";
...
function assertCompatibleHostInfo(value: unknown, expectedSessionId: string): void {
  ...
  if (
    protocolVersion !== 1 ||
    (info?.type !== undefined && info.type !== "vulcan-browser-host") ||
...
export function resolveBrowserHostPipePath(env: NodeJS.ProcessEnv = process.env): string | null {
  const configured =
    env.VULCAN_BROWSER_HOST_PIPE_PATH?.trim() || env.VULCAN_BROWSER_USE_PIPE_PATH?.trim();
...
export function resolveBrowserHostCapability(env: NodeJS.ProcessEnv = process.env): string | null {
  const direct = env.VULCAN_BROWSER_HOST_CAPABILITY?.trim();
  if (direct && Buffer.byteLength(direct, "utf8") >= 32) return direct;

  const rawFd = env.VULCAN_BROWSER_HOST_CAPABILITY_FD?.trim();
