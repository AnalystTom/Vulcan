...
import { MessageId, ThreadId, TurnId } from "@vulcan/contracts";
...
const testLayer = Layer.mergeAll(ProfileStatsQueryLive, ProfileStatsArchiveLive).pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "vulcan-profile-stats-archive-test-",
    }),
  ),
      ...
      Effect.gen(function* () {
        ...
        yield* sql`
          INSERT INTO external_mcp_integrations (
            ...
            'integration-purge', 'Purge integration', 'vulcan.external-mcp', 'other',
