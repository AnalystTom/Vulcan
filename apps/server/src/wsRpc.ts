...
} from "@vulcan/shared/threadDetailEvents";
...
import { discoverSkillsCatalog, vulcanSkillsDir } from "./provider/skillsCatalog";
...
import { FactoryTraceSource } from "./factoryTrace/Services/FactoryTraceSource";
...
class WsRequestAdmissionMiddleware extends RpcMiddleware.Service<WsRequestAdmissionMiddleware>()(
  "vulcan/WsRequestAdmissionMiddleware",
  ...
  AdmittedWsFeatureRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      return AdmittedWsFeatureRpcGroup.of({
              ...
              Effect.gen(function* () {
                ...
                const registerCheckout = Effect.gen(function* () {
                  yield* Queue.offer(queue, {
                    ...
                    kind: "phase",
                    phase: "registering",
                    message: "Adding project to Vulcan",
                  });
        ...
        [WS_METHODS.factoryTraceStatus]: (input) =>
          rpcEffect(factoryTrace.status(input), "Failed to locate the factory trace"),

        [WS_METHODS.factoryTraceListSessions]: (input) =>
          rpcEffect(factoryTrace.listSessions(input), "Failed to list factory sessions"),

        [WS_METHODS.factoryTraceReadSession]: (input) =>
          rpcEffect(factoryTrace.readSession(input), "Failed to read the factory session"),
          ...
          rpcEffect(
            Effect.tryPromise(() =>
              discoverSkillsCatalog({
                cwd: input.cwd ?? null,
                homeDir: config.homeDir,
                vulcanBaseDir: config.baseDir,
                includeDuplicateOrigins: true,
              }),
            ).pipe(
              Effect.map((skills) => ({
                skills,
                vulcanSkillsDir: vulcanSkillsDir(config.baseDir),
              })),
            ),
    ...
    Effect.gen(function* () {
      ...
      yield* router.add(
        ...
        Effect.gen(function* () {
          ...
          // The desktop app fetches cross-origin (vulcan://app); reflect only
