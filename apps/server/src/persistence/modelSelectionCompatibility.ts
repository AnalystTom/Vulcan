...
import { MODEL_OPTIONS_BY_PROVIDER } from "@vulcan/contracts";
...
// Imported instance ids may be runtime names rather than Vulcan provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Vulcan writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Vulcan stores canonical provider/options objects.
