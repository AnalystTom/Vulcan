...
import Migration0095 from "./Migrations/095_BotComms.ts";
...
/**
 * A tracker identity that a *released* Vulcan build wrote for a migration whose
 ...
 * does the migration Vulcan registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Vulcan's latest ID,
 * every Vulcan migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Vulcan's lineage and
 ...
 *
 * Divergences caused by Vulcan renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Vulcan build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Vulcan migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Vulcan build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Vulcan lineage (legacy import); re-running migrations from the divergence point",
