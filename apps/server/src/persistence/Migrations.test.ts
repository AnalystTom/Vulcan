...
layer("reconcileMigrationLineage", (it) => {
  ...
  // mark is at or beyond Vulcan's latest migration ID. The migrator's max-ID
  // gate then skips every Vulcan migration — including the #032 self-heal —
  // and startup crashes on the missing env_mode column.
  it.effect("re-runs skipped migrations when an imported tracker outruns Vulcan's latest ID", () =>
    Effect.gen(function* () {
      ...
      // Record a foreign lineage from 17 through past Vulcan's latest ID.
      const latestVulcanId = Math.max(...migrationEntries.map(([id]) => id));
      for (let id = 17; id <= latestVulcanId + 3; id++) {
        yield* sql`
      ...
      // The tracker now mirrors the Vulcan lineage exactly; foreign rows are gone.
  ...
  it.effect("refuses writable migration startup for a newer Vulcan schema", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (${futureId}, 'FutureVulcanMigration')
      `;
