...
/**
 * Channel rows are keyed by `(bot_a_id, bot_b_id)` in sorted order, so inserts and
 * lookups must derive the pair the same way. Returns a tuple so the callers keep
 * their tuple types instead of widening to an array.
 */
const sortedBotPair = <Id extends string>(botIds: readonly [Id, Id]): readonly [Id, Id] =>
  botIds[0] <= botIds[1] ? [botIds[0], botIds[1]] : [botIds[1], botIds[0]];
...
const makeBotCommsRepository = Effect.gen(function* () {
  ...
  const insertChannelRow = SqlSchema.void({
    ...
    execute: ({ id, botIds, now }) => {
      const [a, b] = sortedBotPair(botIds);
      return sql`
  ...
  const requireChannel = (channelId: BotCommsChannelId, operation: string) =>
    findChannelById(channelId).pipe(
      ...
      Effect.flatMap(
        (rowOption): Effect.Effect<BotCommsChannel, BotRepositoryError> =>
  ...
  const upsertChannel: BotCommsRepositoryShape["upsertChannel"] = (input) => {
    const [a, b] = sortedBotPair(input.botIds);
