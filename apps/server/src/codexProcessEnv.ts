...
// Purpose: Builds the exact environment used when Vulcan launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@vulcan/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@vulcan/shared/shell";

import { resolveBaseCodexHomePath, resolveVulcanCodexHomeOverlayPath } from "./codexHomePaths.ts";
import { buildProviderChildEnvironment } from "./providerChildEnvironment.ts";

const CODEX_PROCESS_SHELL_ENV_NAMES = ["PATH", "SSH_AUTH_SOCK"] as const;
const CODEX_OVERLAY_SHARED_STATE_FILES = new Set(["auth.json"]);
const VULCAN_CONFIG_SUPPRESSIONS_FILE = "vulcan-config-suppressions-v1.json";
const VULCAN_MANAGED_MCP_TABLE_HEADER = "[mcp_servers.vulcan]";
export const VULCAN_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS = [
...
export async function readVulcanConfigSuppressions(markerPath: string): Promise<readonly string[]> {
  try {
...
async function writeVulcanConfigSuppressions(
  markerPath: string,
  sectionHeaders: readonly string[],
): Promise<void> {
...
export const VULCAN_MANAGED_CODEX_CONFIG_BEGIN = "# >>> vulcan managed config >>>";
export const VULCAN_MANAGED_CODEX_CONFIG_END = "# <<< vulcan managed config <<<";

export function extractManagedCodexConfigSection(config: string): string | undefined {
  const begin = config.indexOf(VULCAN_MANAGED_CODEX_CONFIG_BEGIN);
  if (begin === -1) {
    return undefined;
  }
  const contentStart = begin + VULCAN_MANAGED_CODEX_CONFIG_BEGIN.length;
  const end = config.indexOf(VULCAN_MANAGED_CODEX_CONFIG_END, contentStart);
  if (end === -1) {
    return undefined;
  }
...
function appendManagedCodexConfigSection(config: string, section: string): string {
  ...
  const managedMcpTableName = normalizeTomlTableHeaderName(VULCAN_MANAGED_MCP_TABLE_HEADER);
  const tables: string[] = [];
  ...
  for (const table of splitTomlTables(section.trim())) {
    ...
    if (normalizeTomlTableHeaderName(header) === managedMcpTableName) {
      // The session-scoped gateway entry is authoritative inside Vulcan's
      // overlay. The user's source config remains untouched.
      overlayConfig = removeTomlTableNamespace(overlayConfig, VULCAN_MANAGED_MCP_TABLE_HEADER);
      tables.push(table);
      continue;
    }
...
async function prepareVulcanCodexHomeOverlayUnlocked(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveVulcanCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  const suppressionMarkerPath = path.join(overlayHomePath, VULCAN_CONFIG_SUPPRESSIONS_FILE);
  const suppressedSections = [
    ...new Set([
      ...
      ...(await readVulcanConfigSuppressions(suppressionMarkerPath)),
    ]),
  ...
  await writeVulcanConfigSuppressions(suppressionMarkerPath, suppressedSections);

  return overlayHomePath;
}

async function prepareVulcanCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  ...
  return serializeCodexOverlayPreparation(overlayHomePath, () =>
    prepareVulcanCodexHomeOverlayUnlocked(input),
  );
}
...
export async function buildCodexProcessEnv(
  ...
  const overlayHomePath = await prepareVulcanCodexHomeOverlay({
    env: baseEnv,
