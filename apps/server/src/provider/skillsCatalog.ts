...
//          walking) plus the unified cross-provider skills catalog backing Vulcan
//          portable skills. Aggregates `~/.vulcan/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureVulcanSkillsDir

import * as fs from "node:fs/promises";
import * as nodePath from "node:path";

import type { ProviderKind, ProviderSkillDescriptor } from "@vulcan/contracts";
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Vulcan base dir (usually `~/.vulcan`); skills live in `{base}/skills`. */
  readonly vulcanBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Vulcan. */
  readonly includeVulcanRoot?: boolean;
}
...
const ensuredVulcanSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredVulcanSkillsDirs.clear();
}

export function vulcanSkillsDir(vulcanBaseDir: string): string {
  return nodePath.join(vulcanBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureVulcanSkillsDir(vulcanBaseDir: string): Promise<string> {
  const dir = vulcanSkillsDir(vulcanBaseDir);
  if (ensuredVulcanSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredVulcanSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  vulcan: {
    homeRoots: (input) => [vulcanSkillsDir(input.vulcanBaseDir)],
    projectRootNames: [".vulcan"],
  },
  codex: {
    // Keep Vulcan's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Vulcan as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeVulcanRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeVulcanRoot && !ordered.includes("vulcan")) {
    ordered.push("vulcan");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeVulcanRoot || origin !== "vulcan");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeVulcanRoot && origin === "vulcan") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeVulcanRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureVulcanSkillsDir(input.vulcanBaseDir);
    const roots = [
