...
describe("claudeProcessEnv", () => {
  ...
  it("does not grant Vulcan control-plane authority to Claude", () => {
    const result = buildClaudeProcessEnv({
      env: {
        ANTHROPIC_API_KEY: "api-key-auth",
    ...
    assert.equal(result.ANTHROPIC_API_KEY, "api-key-auth");
    assert.equal(result.VULCAN_AUTH_TOKEN, undefined);
    assert.equal(result.VULCAN_BROWSER_USE_PIPE_PATH, undefined);
