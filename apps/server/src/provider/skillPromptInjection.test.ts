...
const vulcanSkillPath = "/Users/me/.vulcan/skills/reviewer/SKILL.md";
...
describe("shouldInlineSkillForProvider", () => {
  it("skips codex-native and vulcan roots for codex but inlines foreign provider roots", () => {
    // Codex loads .codex roots natively and ~/.vulcan/skills via the extra
    // skill root registered at session start.
    expect(shouldInlineSkillForProvider("codex", vulcanSkillPath)).toBe(false);
  ...
  it("inlines only Vulcan-owned paths for cursor", () => {
    expect(shouldInlineSkillForProvider("cursor", vulcanSkillPath)).toBe(true);
  ...
  it("inlines everything except .claude paths for claudeAgent", () => {
    ...
    expect(shouldInlineSkillForProvider("claudeAgent", vulcanSkillPath)).toBe(true);
  ...
  it("inlines cross-provider paths for pi but not pi-native skills", () => {
    expect(shouldInlineSkillForProvider("pi", vulcanSkillPath)).toBe(true);
  ...
  it("always inlines for providers without native skill support", () => {
    for (const provider of ["antigravity", "grok", "kilo", "opencode"] as const) {
      expect(shouldInlineSkillForProvider(provider, vulcanSkillPath)).toBe(true);
...
describe("buildInlineSkillInstructions", () => {
  it("inlines skill content for non-native providers and skips unreadable paths", async () => {
    ...
    const skillDir = path.join(root, ".vulcan", "skills", "reviewer");
    try {
      ...
      const text = await buildInlineSkillInstructions({
        ...
        skills: [
          ...
          { name: "missing", path: path.join(root, ".vulcan", "skills", "missing", "SKILL.md") },
        ],
        maxChars: 10_000,
      });
  ...
  it("does not inline vulcan-rooted skills for codex (covered by the extra skill root)", async () => {
    const text = await buildInlineSkillInstructions({
      provider: "codex",
      skills: [{ name: "reviewer", path: vulcanSkillPath }],
      maxChars: 10_000,
    });
    expect(text).toBe("");
  });
