...
import {
  VULCAN_HARNESS_POLICY_VERSION,
  takeVulcanHarnessPolicyForProviderSession,
} from "../../agentGateway/harnessPolicy.ts";
import { buildOpenCodeMcpServer, VULCAN_MCP_SERVER_NAME } from "../../agentGateway/mcpInjection.ts";
...
interface OpenCodeSessionContext {
  ...
  /** Permission request ids resolved by Vulcan policy and never surfaced to the UI. */
...
const installOpenCodeGatewayMcp = Effect.fn("installOpenCodeGatewayMcp")(function* (input: {
  ...
  const status = result.data?.[VULCAN_MCP_SERVER_NAME];
  ...
  return yield* new OpenCodeRuntimeError({
    ...
    detail:
      status?.status === "failed"
        ? `${input.displayName} Vulcan MCP connection failed: ${status.error}`
        : `${input.displayName} Vulcan MCP connection did not become ready.`,
  });
});
...
function isMatchingHarnessPolicyDelivery(
  ...
  return (
    ...
    delivery.policyVersion === VULCAN_HARNESS_POLICY_VERSION &&
    ...
    ...(input.harnessPolicyDelivered
      ? {
          harnessPolicyDelivery: {
            ...
            policyVersion: VULCAN_HARNESS_POLICY_VERSION,
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (context.activeTurnSawFinalAssistant) {
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "vulcan.opencode.deferred-idle-completion",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "vulcan.opencode.deferred-idle-local-part",
                    event: raw,
                  },
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "vulcan.opencode.deferred-idle-completion-retry",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "vulcan.opencode.deferred-idle-local-part-retry",
                    event: raw,
                  },
                  totalCostUsd: context.latestTurnCostUsd,
                });
            ...
            const completed = yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "vulcan.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "vulcan.opencode.prompt.recovery",
        ...
        yield* Effect.gen(function* () {
          ...
          Effect.flatMap(() =>
            Effect.gen(function* () {
              ...
              const completed = yield* completeOpenCodeTurn(context, {
                turnId: input.turnId,
                raw: { source: "vulcan.opencode.prompt.watchdog" },
                errorMessage: message,
              });
              if (!completed) return;
              yield* emit(context, {
        ...
        switch (event.type) {
          ...
          case "permission.replied": {
            if (context.policyResolvedPermissionIds.has(event.properties.requestID)) {
              // Vulcan policy resolved this request; nothing was surfaced to the UI.
              break;
            }
          ...
          // agent loop. Mirror them into Vulcan's canonical transcript stream.
                  ...
                  Effect.gen(function* () {
                      ...
                      gatewayControlAvailable = yield* installOpenCodeGatewayMcp({
                        ...
                        Effect.catchCause((cause) =>
                          Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                            Effect.andThen(
                              Effect.logWarning(
                                `${adapterConfig.displayName} could not install thread-scoped Vulcan MCP control`,
                                Cause.squash(cause),
                              ),
                            ),
                            Effect.as(false),
                    ...
                    const createSessionId = resumedSessionId
                      ? // A resumed provider may still be executing an interrupted Plan turn.
                        // Install the read-only ruleset until Vulcan dispatches a new turn with a
                        ...
                        // without ever emitting a permission request for Vulcan to reject.
      ...
      const sendTurnImpl = Effect.fn("sendTurn")(function* (
        ...
        const harnessPolicy = takeVulcanHarnessPolicyForProviderSession(
          {
        ...
        // Always pin Vulcan's interaction mode to OpenCode's primary agent.
        // Otherwise a user config with default agent=plan (or a stale options.agent=plan
        // after leaving Vulcan plan mode) can trap default turns in plan mode.
        ...
        if (provider === "kilo") {
          ...
          // server rather than by one long-lived HTTP request. Keep Vulcan's event
