...
import {
  ...
  resolveApiModelId,
  trimOrNull,
} from "@vulcan/shared/model";
import { buildClaudeSubagentPrompt } from "@vulcan/shared/agentMentions";
...
import { renderVulcanHarnessPolicy } from "../../agentGateway/harnessPolicy.ts";
...
// from these hints (see extractSubagentIdentityHints in @vulcan/shared/subagents).
...
export const buildEmbeddedClaudeSystemPromptAppend = (gatewayControlAvailable: boolean) =>
  [
    "You are running inside Vulcan, a coding app that embeds the Claude Agent SDK.",
    ...
    renderVulcanHarnessPolicy({ gatewayControlAvailable }),
  ].join("\n");
        ...
        if (Exit.isFailure(exit)) {
          if (hasPendingUserInterrupt(context) || isClaudeInterruptedCause(exit.cause)) {
            ...
            if (isClaudeMissingResumeConversationCause(exit.cause)) {
              ...
              // starts a fresh Claude session and bootstraps Vulcan's retained
    ...
    const startSessionUnlocked: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const settings = {
          ...
          // Vulcan's safer 200k budget explicit unless the thread opts into 1M.
