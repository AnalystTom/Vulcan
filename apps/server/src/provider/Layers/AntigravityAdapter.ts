...
import {
  ...
  VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV,
  VULCAN_AGENT_GATEWAY_URL_ENV,
} from "../../agentGateway/mcpInjection.ts";
import {
  type VulcanHarnessPolicyDeliveryState,
...
/**
 * Hook output when capture is inactive (the session is not Vulcan-managed).
...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    return `if not defined VULCAN_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo ${fallback}) else (set "ELECTRON_RUN_AS_NODE=1" && ${invocation})`;
  }
  return `if [ -z "\${VULCAN_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '${fallback}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.VULCAN_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.VULCAN_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "vulcan-capture",
        description: "Streams Antigravity CLI lifecycle events to Vulcan when requested.",
      },
      null,
      2,
    )}\n`,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [VULCAN_AGENT_GATEWAY_URL_ENV, VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [VULCAN_AGENT_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
        [VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: input.gatewayBootstrapToken!,
      }
    : {};
  return buildProviderChildEnvironment({
    ...
    inheritedVulcanKeys: [
      ...
      "VULCAN_ANTIGRAVITY_HOOK_DECISION",
      ...gatewayKeys,
    ],
    overrides: {
      VULCAN_ANTIGRAVITY_EVENTS: input.eventFile,
...
export function buildAntigravityTurnPrompt(
  ...
  const harnessPolicy = takeVulcanHarnessPolicyForProviderSession(state, {
    provider: PROVIDER,
...
export function parseAntigravityCliModelLabel(
  ...
  // Vulcan never treats `slug\tName` as a single model id at dispatch.
        ...
        yield* Effect.tryPromise({
          try: () =>
            ...
            new ProviderAdapterRequestError({
              ...
              detail: messageFromCause(cause, "Failed to install the Vulcan capture hook."),
              cause,
            }),
        });
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const runDir = yield* Effect.tryPromise({
          try: () => fs.mkdtemp(path.join(os.tmpdir(), "vulcan-antigravity-")),
          catch: (cause) =>
            new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "turn/prepare",
        ...
        if (gatewaySessionLease && !gatewayBootstrapToken) {
          ...
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
            detail: "The Vulcan gateway credential is no longer active for this provider turn.",
          });
        }
