...
import { decodeOutboundJson, decodeOutboundText, outboundHttp } from "@vulcan/shared/outboundHttp";
import {
  Cause,
  DateTime,
  Deferred,
...
import { buildAcpVulcanMcpServers } from "../../agentGateway/mcpInjection.ts";
import {
...
export const takeGrokVulcanHarnessPolicyTextPart = (
...
const GROK_ACP_DEBUG_ENV = "VULCAN_GROK_ACP_DEBUG";
const VULCAN_GROK_ACP_DEBUG_ENV = "VULCAN_GROK_ACP_DEBUG";
...
// override with VULCAN_GROK_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const GROK_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "VULCAN_GROK_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const GROK_PLAN_MODE_PROMPT_PREFIX = [
  "Vulcan requested Grok's native plan mode.",
...
const GROK_PLAN_GUARD_HOOK_CALLBACK_ID = "vulcan-plan-guard";
...
export function buildGrokPromptMeta(interactionMode: ProviderInteractionMode): {
  ...
  // the provider state when Vulcan sends the desired mode again.
...
export function resolveGrokPlanHookResponse(
  ...
  return {
    decision: "deny",
    systemMessage: `Vulcan Plan mode blocks the mutating or unknown Grok tool "${toolName || "unknown"}".`,
  };
}
...
function isGrokAcpDebugEnabled(): boolean {
  return (
    ...
    process.env[VULCAN_GROK_ACP_DEBUG_ENV] === "1" ||
...
export function makeGrokAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // present, every session gets the vulcan_* MCP tools.
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeGrokAcpRuntime({
            ...
            clientInfo: { name: "Vulcan", version: "0.0.0" },
                ...
                Effect.gen(function* () {
                  ...
                  if (planMarkdown !== undefined) {
                    ...
                    if (
                      ...
                      ctx.lastPlanFingerprint = planMarkdown;
                      // The extension response must reach Grok before Vulcan cancels the
    ...
    const startGrokTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeGrokVulcanHarnessPolicyTextPart(
          ctx,
              ...
              Effect.gen(function* () {
                ...
                if (terminalPlanMarkdown !== undefined) {
                  ...
                  yield* offerRuntimeEvent(ctx.lifecycleGeneration, {
                    ...
                    raw: {
                      source: "acp.jsonrpc",
                      method: "vulcan.grok.terminal-plan-response",
                      payload: result,
                    },
                  });
                }
