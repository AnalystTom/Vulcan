...
describe("Claude Vulcan harness policy", () => {
  it("advertises scoped MCP additively when credentials are available", () => {
    const text = buildEmbeddedClaudeSystemPromptAppend(true);
    assert.include(text, VULCAN_HARNESS_POLICY_MARKER);
    assert.include(text, "Use the vulcan_* tools");
    assert.notInclude(text, "Vulcan MCP control is unavailable");
  });

  it("stays truthful when scoped MCP credentials are absent", () => {
    const text = buildEmbeddedClaudeSystemPromptAppend(false);
    assert.include(text, VULCAN_HARNESS_POLICY_MARKER);
    assert.include(text, "Vulcan MCP control is unavailable");
  });
});
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("injects the canonical Vulcan browser MCP into an Opus 4.8 session", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        systemPrompt.append ?? "",
        "exact thread-scoped Electron page Vulcan surfaces to the user",
      );
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
  ...
  it.effect("loads Claude filesystem settings sources for SDK sessions", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(systemPrompt.append ?? "", VULCAN_HARNESS_POLICY_MARKER);
      assert.include(systemPrompt.append ?? "", "Vulcan is the host and harness");
      // This characterization harness intentionally omits gateway credentials.
      assert.include(systemPrompt.append ?? "", "Vulcan MCP control is unavailable");
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
...
`;
  ...
  it.effect("sets plan permission mode on sendTurn when interactionMode is plan", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(promptText ?? "", "Vulcan plan mode is active.");
