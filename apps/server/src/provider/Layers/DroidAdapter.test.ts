...
import {
  ...
  takeDroidVulcanHarnessPolicyTextPart,
} from "./DroidAdapter.ts";

describe("Droid Vulcan harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeDroidVulcanHarnessPolicyTextPart(state, true)?.text).toContain(
      VULCAN_HARNESS_POLICY_MARKER,
    );
    expect(takeDroidVulcanHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
