...
import { ServerProviderUpdateError } from "@vulcan/contracts";
import { parseCodexConfigModelProvider } from "@vulcan/shared/codexConfig";
import { decodeJsonResult } from "@vulcan/shared/schemaJson";
import { prepareWindowsSafeProcess } from "@vulcan/shared/windowsProcess";
...
const DISABLED_PROVIDER_STATUS_MESSAGE = "Provider is disabled in Vulcan settings.";
...
function parseCursorAuthStatusFromOutput(result: CommandResult): {
  ...
  if (result.code === 0) {
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Cursor Agent is installed, but Vulcan could not verify authentication status.",
    };
  }
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      ...(hasApiKey
        ...
        : {
            message:
              "Droid CLI is installed. Vulcan can use the CLI's cached device-pairing login; run `droid` to authenticate locally if needed, or set FACTORY_API_KEY.",
          }),
    } satisfies ServerProviderStatus;
  });
...
export const checkPiProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    // Pi itself is SDK-backed in Vulcan. Keep this CLI probe advisory so health
    ...
    if (versionProbe.outcome === "missing" || versionProbe.outcome === "failure") {
      ...
      return {
        ...
        message:
          versionProbe.outcome === "missing"
            ? "Pi SDK is bundled, but the Pi CLI (`pi`) is not on PATH, so Vulcan could not verify the installed CLI version."
    ...
    if (versionProbe.outcome === "timeout") {
      return {
        ...
        checkedAt,
        message:
          "Pi SDK is bundled, but the CLI health check timed out before Vulcan could verify the installed version.",
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      message: configuredAgentDir
        ? `Pi CLI is installed. Vulcan will use Pi agent dir ${configuredAgentDir}.`
  ...
  Effect.gen(function* () {
      ...
      return {
        ...
        message:
          versionProbe.outcome === "missing"
            ...
            : `Failed to execute OMP CLI health check: ${error instanceof Error ? error.message : String(error)}.`,
      } satisfies ServerProviderStatus;
    }
    ...
    if (versionProbe.outcome === "timeout") {
      return {
        ...
        authStatus: "unknown" as const,
        checkedAt,
        message: "OMP CLI is installed but failed to run. Timed out while running command.",
      } satisfies ServerProviderStatus;
    }
    ...
    if (versionProbe.outcome === "nonzero") {
      ...
      return {
        ...
        checkedAt,
        message: detail
          ? `OMP CLI is installed but failed to run. ${detail}`
          : "OMP CLI is installed but failed to run.",
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      checkedAt,
      message: configuredAgentDir
        ? `OMP CLI is installed. Vulcan will use OMP agent dir ${configuredAgentDir}.`
        : "OMP CLI is installed. Models come from ~/.omp/agent/models.yml.",
    } satisfies ServerProviderStatus;
  });
  ...
  Effect.gen(function* () {
    ...
    if (
      ...
      return {
        ...
        version: parsedVersion,
        checkedAt,
        message: `Antigravity CLI ${parsedVersion} is too old for Vulcan. Upgrade to ${MINIMUM_ANTIGRAVITY_CLI_VERSION} or newer.`,
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      version: parsedVersion,
      checkedAt,
      message: "Antigravity CLI is installed, but Vulcan could not verify login by listing models.",
    } satisfies ServerProviderStatus;
  });
  ...
  Effect.gen(function* () {
    ...
    if (Option.isNone(modelsProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Cursor Agent is authenticated, but model discovery timed out before Vulcan could verify available models.",
      } satisfies ServerProviderStatus;
    }
      ...
      const updateProvider: ProviderHealthShape["updateProvider"] = Effect.fn(
        ...
        if (!isProviderEnabledForSettings(provider, settings)) {
          return yield* new ServerProviderUpdateError({
            provider,
            reason: "Provider is disabled in Vulcan settings.",
          });
        }
          ...
          const finalProviders = yield* setProviderUpdateState(
            ...
            makeUpdateState({
              ...
              message: stillOutdated
                ? `Update command completed, but Vulcan still detects an outdated provider version${stillOutdatedVersions}.`
                : "Provider updated.",
              output: output ? output.slice(0, UPDATE_OUTPUT_MAX_BYTES) : null,
            }),
          );
