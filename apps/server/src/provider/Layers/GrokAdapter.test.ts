...
import { TurnId } from "@vulcan/contracts";
...
import {
  ...
  takeGrokVulcanHarnessPolicyTextPart,
} from "./GrokAdapter.ts";

describe("Grok Vulcan harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeGrokVulcanHarnessPolicyTextPart(state, true)?.text).toContain(
      VULCAN_HARNESS_POLICY_MARKER,
    );
    expect(takeGrokVulcanHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
...
describe("Grok native plan approval", () => {
  it("adds Plan instructions without sending a pager-only slash command as model text", () => {
    ...
    ).toMatch(/^Vulcan requested Grok's native plan mode\./u);
  });
  ...
  it("backs native Plan mode with a fail-closed pre-tool hook", () => {
    expect(
      resolveGrokPlanHookResponse("plan", {
        hookCallbackId: "vulcan-plan-guard",
  ...
  it("keeps native plan mode gated after Vulcan captures the plan", () => {
    expect(makeGrokExitPlanModeCapturedResponse()).toEqual({
      outcome: "cancelled",
      feedback:
        "Vulcan captured this plan for user review. Do not revise or implement it now. End this turn and wait for the user's next message.",
    });
  });
...
describe("Grok native user questions", () => {
  ...
  it("maps Vulcan answers to Grok's question-text keyed response", () => {
