...
import type { ChatAttachment } from "@vulcan/contracts";
import { resolveWindowsComSpec } from "@vulcan/shared/windowsProcess";
...
describe("toOpenCodeFileParts", () => {
  it("materializes image attachments as SDK file parts", () => {
    ...
    expect(
      toOpenCodeFileParts({
        ...
        resolveAttachmentPath: () => "/tmp/vulcan-attachments/screenshot.png",
      }),
    ).toEqual([
      {
        type: "file",
        ...
        url: "file:///tmp/vulcan-attachments/screenshot.png",
      },
    ]);
  });
  ...
  it("leaves generic files for prompt-path projection", () => {
    ...
    expect(
      toOpenCodeFileParts({
        attachments: [attachment],
        resolveAttachmentPath: () => "/tmp/vulcan-attachments/notes.docx",
      }),
    ).toEqual([]);
  });
});
...
describe("buildOpenCodeServerProcessEnv", () => {
  ...
  it("strips inherited Vulcan authority from managed server processes", () => {
    const env = buildOpenCodeServerProcessEnv({
      baseEnv: {
        OPENAI_API_KEY: "provider-key",
        VULCAN_AUTH_TOKEN: "server-secret",
        VULCAN_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
      },
    });

    expect(env.OPENAI_API_KEY).toBe("provider-key");
    expect(env.VULCAN_AUTH_TOKEN).toBeUndefined();
        ...
        Effect.gen(function* () {
          ...
          const connectionFiber = yield* runtime
            .connectToOpenCodeServer({
              ...
              poolIsolationKey: "vulcan-kilo-thread",
            })
        ...
        Effect.gen(function* () {
          ...
          const first = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "vulcan-thread-a",
            })
            .pipe(Effect.provideService(Scope.Scope, firstScope));
          const second = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "vulcan-thread-b",
            })
            .pipe(Effect.provideService(Scope.Scope, secondScope));
