// Verifies the current Vulcan ACP boundary against an official-SDK subprocess.
...
function createFixtureLog(): string {
  const directory = mkdtempSync(path.join(os.tmpdir(), "vulcan-acp-conformance-"));
...
function runtimeLayer(logPath: string, env: Record<string, string> = {}) {
  return AcpSessionRuntime.layer({
    spawn: {
      ...
      env: {
        VITEST: "true",
        VULCAN_ACP_CONFORMANCE_LOG_PATH: logPath,
        ...env,
      },
    },
    cwd: process.cwd(),
    clientCapabilities: {
      _meta: {
        ...
        nested: { source: "vulcan" },
      },
    },
    clientInfo: { name: "vulcan-conformance-test", version: "0.0.0" },
    authMethodId: "test",
    authenticateMeta: {
      primitive: 11,
      nested: { source: "vulcan-auth" },
    },
  });
}
...
describe("official ACP SDK conformance at the current Vulcan boundary", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        runtimeLayer(logPath, {
          VULCAN_ACP_CONFORMANCE_MALFORMED_PREFIX: "1",
        }),
      ),
      Effect.scoped,
      Effect.provide(NodeServices.layer),
  ...
  it.effect("round-trips extension requests and delivers extension notifications", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runtime.notify("conformance/notice", {
        primitive: false,
        nested: { source: "vulcan-notice" },
      });
      const response = yield* runtime.request("conformance/echo", {
        primitive: 42,
        nested: { source: "vulcan-request" },
      });
  ...
  it.effect("cancels an extension request through official-SDK $/cancel_request", () => {
    ...
    return Effect.gen(function* () {
      ...
      const request = yield* runtime
        .request("conformance/wait-for-generic-cancel", { source: "vulcan" })
        .pipe(Effect.forkChild);
...
describe("official ACP SDK client against the official SDK mock agent", () => {
  test("completes initialize, authentication, session, prompt, updates, stop, and teardown", async () => {
    ...
    const child = spawn(process.execPath, [officialMockFixturePath], {
      ...
      env: {
        ...process.env,
        VULCAN_ACP_REQUEST_LOG_PATH: requestLogPath,
        VULCAN_ACP_EXIT_LOG_PATH: exitLogPath,
        VULCAN_ACP_PROMPT_RESPONSE_TEXT: "mock says héllo 👋",
      },
