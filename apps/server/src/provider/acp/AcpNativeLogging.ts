import type { ProviderKind, ThreadId } from "@vulcan/contracts";
...
const SENSITIVE_ENTRY_NAMES = new Set(["authorization", "vulcan_agent_gateway_token"]);

function redactSecretString(value: string): string {
  return value
    .replace(/(\bBearer\s+)[^\s"',}\]]+/gi, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(/(VULCAN_AGENT_GATEWAY_TOKEN\s*=\s*)[^\s"',}\]]+/g, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(/("VULCAN_AGENT_GATEWAY_TOKEN"\s*:\s*")[^"]*/g, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(
      /("name"\s*:\s*"VULCAN_AGENT_GATEWAY_TOKEN"\s*,\s*"value"\s*:\s*")[^"]*/g,
