import type { ProviderUserInputAnswers, UserInputQuestion } from "@vulcan/contracts";
...
const VULCAN_PLAN_REVIEW_FEEDBACK =
...
/**
 * Vulcan owns the approval step after the planning turn settles. Returning a
...
export function makeGrokExitPlanModeCapturedResponse(): {
  ...
  return {
    outcome: "cancelled",
    feedback: VULCAN_PLAN_REVIEW_FEEDBACK,
  };
}

/** Vulcan has already received an implementation turn, so Grok may leave its native gate. */
