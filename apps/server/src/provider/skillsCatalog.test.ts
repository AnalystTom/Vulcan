...
import type { ProviderSkillDescriptor } from "@vulcan/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "vulcan-skills-catalog-"));
  homeDir = path.join(root, "home");
  vulcanBaseDir = path.join(homeDir, ".vulcan");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Vulcan skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, vulcanBaseDir });
    await expect(access(path.join(vulcanBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from vulcan and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(vulcanBaseDir, "skills", "portable"), "portable", "Vulcan skill");
    ...
    const skills = await discoverSkillsCatalog({ homeDir, vulcanBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("vulcan");
  ...
  it("includes user and matching project Claude plugins but excludes other projects", async () => {
    ...
    const skills = await discoverSkillsCatalog({ cwd, homeDir, vulcanBaseDir });
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, vulcanBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Vulcan for that provider", async () => {
    await writeSkill(path.join(vulcanBaseDir, "skills", "shared"), "shared", "Vulcan copy");
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(path.join(vulcanBaseDir, "skills", "only-vulcan"), "only-vulcan", "Fallback");

    const codexView = await discoverSkillsCatalog({ homeDir, vulcanBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-vulcan")).toBe(true);

    // A provider without its own copy resolves the Vulcan fallback.
    ...
    expect(claudeShared?.scope).toBe("vulcan");
  });

  it("uses documented provider alias roots before Vulcan fallbacks", async () => {
    await writeSkill(path.join(vulcanBaseDir, "skills", "shared"), "shared", "Vulcan copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(vulcanBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, vulcanBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(vulcanBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, vulcanBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, vulcanBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .vulcan skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".vulcan", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(vulcanBaseDir, "skills", "portable"), "portable", "Vulcan");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("vulcan");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(vulcanBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(vulcanBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "vulcan"), descriptor("extra", "vulcan")],
    });
