...
//          that makes Vulcan catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@vulcan/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.vulcan/skills`, which Vulcan registers at session start
    ...
    case "cursor":
      ...
      // only Vulcan-owned paths need inlining.
      return segments.has(".vulcan");
    case "claudeAgent":
