...
import { DEFAULT_GIT_TEXT_GENERATION_MODEL } from "@vulcan/contracts";
import { sanitizeGeneratedThreadTitle } from "@vulcan/shared/chatThreads";
import { resolveCodexHome } from "@vulcan/shared/codexConfig";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@vulcan/shared/git";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    const filePath = path.join(tempDir, `vulcan-${prefix}-${process.pid}-${randomUUID()}.tmp`);
  ...
  const prepareIsolatedCodexHome = (
    ...
    Effect.gen(function* () {
      ...
      const isolatedHomePath = path.join(
        tempDir,
        `vulcan-codex-home-${process.pid}-${randomUUID()}`,
      );
