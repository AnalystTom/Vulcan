...
} from "@vulcan/shared/git";
import { parseGitHubRepositoryNameWithOwnerFromRemoteUrl } from "@vulcan/shared/githubRepository";
import { summarizeUnifiedPatchTotals } from "@vulcan/shared/unifiedPatchStats";
import { resolveWorktreeHandoffIntent } from "@vulcan/shared/worktreeHandoff";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `vulcan/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.gen(function* () {
  ...
  const runPrStep = (
    ...
    Effect.gen(function* () {
      ...
      const bodyFile = path.join(tempDir, `vulcan-pr-body-${process.pid}-${randomUUID()}.md`);
      yield* fileSystem
  ...
  const handoffThread: GitManagerShape["handoffThread"] = Effect.fnUntraced(function* (input) {
    ...
    if (input.targetMode === "local") {
      ...
      const preservedLocalStash = yield* stashWorkingTree(
        input.cwd,
        `vulcan preserve local handoff ${randomUUID()}`,
      );
      const sourceStash = yield* stashWorkingTree(
        input.worktreePath,
        `vulcan handoff to local ${randomUUID()}`,
      );
    ...
    const sourceStash = yield* stashWorkingTree(
      input.cwd,
      `vulcan handoff to worktree ${randomUUID()}`,
    );
