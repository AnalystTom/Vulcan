  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("vulcan: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "vulcan: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vulcan/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vulcan/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "vulcan/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "vulcan/tmp-working",
          newBranch: "vulcan/feat/session",
        });

        expect(renamed.branch).toBe("vulcan/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "vulcan/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "vulcan/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("vulcan/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vulcan/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vulcan/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "vulcan/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "vulcan/tmp-working" });
        ...
        expect(renamed.branch).toBe("vulcan/feat/session-2");
      }),
    );
  ...
  describe("createGitWorktree", () => {
    ...
    it.effect("creates a branch-backed managed worktree when newBranch is provided", () =>
      Effect.gen(function* () {
        ...
        const result = yield* core.createDetachedWorktree({
          ...
          ref: "HEAD",
          path: wtPath,
          newBranch: "vulcan/abcd1234",
        });

        expect(result.worktree).toEqual({
          path: wtPath,
          ref: expectedHead,
          branch: "vulcan/abcd1234",
        });
        expect(yield* git(wtPath, ["symbolic-ref", "--short", "HEAD"])).toBe("vulcan/abcd1234");
        expect(yield* git(tmp, ["rev-parse", "refs/heads/vulcan/abcd1234"])).toBe(expectedHead);
        ...
        const remainingBranches = yield* git(tmp, ["branch", "--list", "vulcan/abcd1234"]);
    ...
    it.effect("does not report branch creation before preliminary validation", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["branch", "--list", "vulcan/notstarted"])).toBe("");
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(
          core.createDetachedWorktree({
            ...
            ref: "HEAD",
            path: wtPath,
            newBranch: "vulcan/rollback1",
          }),
        );

        expect(Exit.isFailure(result)).toBe(true);
        expect(yield* git(tmp, ["branch", "--list", "vulcan/rollback1"])).toBe("");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:example-org/vulcan.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:example-org/vulcan.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "vulcan/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
