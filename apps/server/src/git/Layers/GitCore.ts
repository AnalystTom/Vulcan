...
import { parseGitHubRepositoryNameWithOwnerFromRemoteUrl } from "@vulcan/shared/githubRepository";
...
const AUTO_DETACHED_WORKTREE_DIRNAME = "vulcan";
const WORKTREE_OWNERSHIP_MARKER = "vulcan-agent-gateway-owner.json";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `vulcan-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
  ...
  Effect.gen(function* () {
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempIndexDir = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `vulcan-git-status-index-${process.pid}-`,
          });
      ...
      Effect.gen(function* () {
        ...
        if (patch.length > 0) {
          yield* Effect.acquireUseRelease(
            Effect.tryPromise({
              try: () => nodeFs.mkdtemp(nodePath.join(tmpdir(), "vulcan-worktree-patch-")),
              catch: (cause) =>
                createGitCommandError(
    ...
    const removeWorktree: GitCoreShape["removeWorktree"] = (input) =>
      Effect.gen(function* () {
        ...
        // vulcan/* branches qualify for reclamation; detached HEADs and
    ...
    const stashAndCheckout: GitCoreShape["stashAndCheckout"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["stash", "push", "-u", "-m", `vulcan: stash before switching to ${input.branch}`],
          {
            timeoutMs: 30_000,
