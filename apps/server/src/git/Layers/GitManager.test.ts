...
import type { GitActionProgressEvent } from "@vulcan/contracts";
import type {
  GitPullRequestCheck,
...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("vulcan-git-remote-");
...
function makeManager(input?: {
  ...
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "vulcan-git-manager-test-",
  });
...
const GitManagerTestLayer = GitCoreLive.pipe(
  Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "vulcan-git-manager-test-" })),
  Layer.provideMerge(NodeServices.layer),
);
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes PR metadata when branch already has an open PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("vulcan-git-manager-");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "vulcan/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.branch).toBe("vulcan/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/example-org/sample-repo/pull/488",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["remote", "add", "fork-seed", forkDir]);
        yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
        yield* runGit(repoDir, ["checkout", "-b", "vulcan/pr-142/statemachine"]);
        yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
        yield* runGit(repoDir, [
          "config",
          "remote.fork-seed.url",
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              "vulcan/pr-142/statemachine": JSON.stringify([]),
  ...
  it.effect("uses the local base template when a cross-repo origin ref is absent", () =>
    Effect.gen(function* () {
      ...
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
      yield* runGit(repoDir, ["checkout", "-b", "vulcan/pr-91/statemachine"]);
      yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
      yield* runGit(repoDir, [
        "config",
        "remote.fork-seed.url",
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/example-org/vulcan/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("vulcan/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("vulcan/pr-92/main");
