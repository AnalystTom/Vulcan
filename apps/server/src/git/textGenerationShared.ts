...
import { MAX_CHAT_THREAD_TITLE_WORDS } from "@vulcan/shared/chatThreads";
...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for Vulcan's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured Vulcan automation creation intents.",
      ...
      "- Vulcan automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed Vulcan heartbeat automation should stop.",
