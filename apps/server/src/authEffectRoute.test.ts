...
import { AuthSessionId } from "@vulcan/contracts";
import {
...
function makeServerAuth(sideEffects: { count: number }): ServerAuthShape {
  ...
  return {
    ...
    authenticateHttpRequest: (request) => {
      ...
      const cookie = request.cookies.vulcan_session === "cookie-token";
      if (!bearer && !cookie) {
    ...
    issueStartupPairingUrl: () =>
      Effect.succeed("https://vulcan.example.test/pair#token=PAIRINGTOKEN"),
  ...
  return {
    ...
    headers: {
      ...
      ...(input.credential === "bearer"
        ...
        : { Cookie: "vulcan_session=cookie-token" }),
  ...
  it("logs out either role and clears the exact cookie with secure public-mode attributes", async () => {
    ...
    await withAuthEffectServer(config, makeServerAuth(sideEffects), async (serverOrigin) => {
      const response = await fetch(
        ...
        mutationRequest({
          origin: "https://vulcan.example.test",
          credential: "cookie",
        }),
      );
      ...
      expect(cookie).toContain("vulcan_session=");
...
describe("binaryUploadEffectRouteLayer", () => {
  it("allows credentialed Canary attachment upload preflights", async () => {
    const config = {
      host: "127.0.0.1",
      attachmentsDir: fs.mkdtempSync(path.join(os.tmpdir(), "vulcan-upload-cors-")),
    } as ServerConfigShape;
    try {
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          expect(response.headers.get("access-control-allow-origin")).toBe("vulcan-canary://app");
  ...
  it("rejects ambient cookie uploads without an origin and accepts explicit bearer auth", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "vulcan-upload-route-"));
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://vulcan.example.test/"),
      attachmentsDir,
      ...
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          const cookieResponse = await fetch(url, {
            method: "POST",
            headers: { Cookie: "vulcan_session=cookie-token" },
