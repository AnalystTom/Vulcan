...
export function remoteAccessPolicyError(
  ...
  if (config.publicUrl && !normalizeHttpsPublicOrigin(config.publicUrl)) {
    return "VULCAN_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://vulcan.example.com).";
  }
  const isPubliclyExposed = isRemoteBind || Boolean(config.publicUrl);
  if (!isPubliclyExposed) return null;
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish Vulcan through VULCAN_PUBLIC_URL/--public-url without VULCAN_AUTH_TOKEN/--auth-token."
      : `Refusing to bind Vulcan to non-loopback host ${config.host ?? "<unspecified>"} without VULCAN_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
  ...
  if (isRemoteBind && !config.publicUrl && !config.allowInsecureRemote) {
    return "Refusing plaintext remote access. Configure an HTTPS reverse-proxy origin with VULCAN_PUBLIC_URL/--public-url, or explicitly accept unencrypted LAN traffic with VULCAN_ALLOW_INSECURE_REMOTE/--allow-insecure-remote.";
  }
  return null;
}
...
export function resolveDefaultChatWorkspaceRoot(input: {
  ...
  return pathApi.join(homeDir, "Documents", "Vulcan");
}
...
export const resolveStaticDir = Effect.fn(function* () {
  ...
  const snapshotDir = process.env.VULCAN_STATIC_DIR?.trim();
  if (snapshotDir) {
