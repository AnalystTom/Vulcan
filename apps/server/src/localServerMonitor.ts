...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "VulcanLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  const server: ServerLocalServerProcess = {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "Vulcan cannot signal this process." }),
  };
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "Vulcan cannot stop this process.",
    };
  }
