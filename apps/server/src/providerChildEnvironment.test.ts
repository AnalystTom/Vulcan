...
describe("buildProviderChildEnvironment", () => {
  it("strips Vulcan control-plane and inherited native capabilities", () => {
    const env = buildProviderChildEnvironment({
      ...
      baseEnv: {
        ...
        VULCAN_AUTH_TOKEN: "control-plane-secret",
        VULCAN_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
        VULCAN_BROWSER_HOST_CAPABILITY: "private-desktop-capability",
  ...
  it("admits only explicitly granted capability keys", () => {
    const env = buildProviderChildEnvironment({
      ...
      inheritedVulcanKeys: ["VULCAN_BROWSER_USE_PIPE_PATH"],
  ...
  it("does not let overlays bypass the capability policy", () => {
    const env = buildProviderChildEnvironment({
      ...
      overrides: {
        ...
        VULCAN_AUTH_TOKEN: "overlaid-control-plane-secret",
        NODE_OPTIONS: "--require=/tmp/inject.js",
      },
    });
  ...
  it.each(["codex", "kilo", "opencode", "pi", "omp"] as const)(
  ...
  it("keeps stripped authority absent in descendants", () => {
    ...
    const descendantScript =
      "process.stdout.write(JSON.stringify({ xai: process.env.XAI_API_KEY, anthropic: process.env.ANTHROPIC_API_KEY, vulcan: process.env.VULCAN_AUTH_TOKEN }))";
