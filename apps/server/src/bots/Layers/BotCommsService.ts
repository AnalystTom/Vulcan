...
import { Deferred, Effect, Layer, Option, PubSub, Result, Stream } from "effect";
...
const make = Effect.gen(function* () {
    ...
    Effect.gen(function* () {
      ...
      if (Result.isFailure(dispatched)) {
        return {
          kind: "failed",
          target,
          error: dispatched.failure.message,
    ...
    Effect.gen(function* () {
      ...
      if (Result.isFailure(dispatched)) {
        yield* updateDelegation({
          id: delegation.id,
          status: "failed",
          result: dispatched.failure.message,
          targetThreadId,
          channelId: channel.id,
          updatedAt: isoNow(),
        });
  ...
  const finishRunningDelegations = (
    threadId: ThreadId,
    outcome: "completed" | "failed" | "interrupted",
  ) =>
    Effect.gen(function* () {
      ...
      for (const delegation of mine) {
        ...
        const result = failed
          ? (reply?.error ??
            `The delegated turn ${outcome === "interrupted" ? "was interrupted" : "failed"}.`)
          : reply.text.length > 0
            ? reply.text
            : "(no reply text)";
        yield* updateDelegation({
  ...
  const listChannels: BotCommsServiceShape["listChannels"] = (input) =>
    commsRepository.listChannels(input.botId === undefined ? {} : { botId: input.botId }).pipe(
  ...
  const listMessages: BotCommsServiceShape["listMessages"] = (input) =>
    commsRepository.listMessages({ channelId: input.channelId, limit: input.limit }).pipe(
      Effect.tap(() =>
        commsRepository
  ...
  const listDelegations: BotCommsServiceShape["listDelegations"] = (input) =>
    commsRepository.listDelegations(input.botId === undefined ? {} : { botId: input.botId }).pipe(
