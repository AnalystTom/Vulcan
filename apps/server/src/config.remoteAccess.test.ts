...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without VULCAN_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://vulcan.example.test/"),
      new URL("https://vulcan.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://vulcan.example.test/"))?.origin).toBe(
      "https://vulcan.example.test",
    );
    for (const value of [
      "http://vulcan.example.test/",
      "https://user:pass@vulcan.example.test/",
      "https://vulcan.example.test/app",
      "https://vulcan.example.test/?query=1",
      "https://vulcan.example.test/#fragment",
    ]) {
