/**
 * AgentGatewayLive - Vulcan app-control MCP tool surface.
 *
 * Implements the `vulcan_*` tools served over `POST /mcp` (streamable HTTP,
 ...
 * a Vulcan thread can list/read/create/steer threads and manage heartbeat
...
import { runtimeModeEscalatesPrivilege } from "@vulcan/shared/runtimeMode";
...
const AGENT_GATEWAY_INSTRUCTIONS =
  "Vulcan tools are thread-scoped. Use browser_* only for Vulcan's shared in-app browser runtime; follow the provider-delivered <vulcan_host_context> for full policy.";
...
export const makeAgentGateway = Effect.gen(function* () {
  ...
  const createThreads: ToolEntry = {
    ...
    definition: {
      name: "vulcan_create_threads",
      description:
        "Create an exact batch of 1–20 standalone Vulcan threads. Worktree threads start on a Vulcan-managed temporary branch pinned at baseRef (or the selected checkout's HEAD) and copy local checkout changes plus .worktreeinclude files when the ref is that checkout's HEAD; on the first turn Vulcan may rename the branch after the prompt and publish it. Validation/preflight failures create nothing and may be corrected with the same requestId; durable retries replay the exact operation.",
      inputSchema: {
        type: "object",
        properties: {
          requestId: {
          ...
          threads: {
            type: "array",
            minItems: 1,
            maxItems: VULCAN_GATEWAY_MAX_THREADS_PER_OPERATION,
            items: {
              type: "object",
              properties: {
                prompt: { type: "string" },
  ...
  const createThread: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "vulcan_create_thread",
      description:
        "Create exactly one standalone Vulcan thread. Worktree threads start on a Vulcan-managed temporary branch pinned at baseRef; on the first turn Vulcan may rename the branch after the prompt and publish it. For two or more threads use one vulcan_create_threads call instead.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  const sendMessage: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "vulcan_send_message",
      description:
        'Send a Vulcan follow-up message to an existing thread. mode "queue" (default) waits for the current turn; "steer" redirects a running turn where the provider supports it (otherwise it is queued).',
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Send a Vulcan message", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const interruptThread: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "vulcan_interrupt_thread",
      description: "Interrupt the running turn of a Vulcan thread.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Interrupt a Vulcan thread", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const threadId = readStringArg(args, "threadId", { required: true })!;
  ...
  const setThreadTitle: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "vulcan_set_thread_title",
      description: "Rename a Vulcan thread.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Rename a Vulcan thread", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const threadId = readStringArg(args, "threadId", { required: true })!;
  ...
  const setThreadArchived: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "vulcan_set_thread_archived",
      description:
        "Archive or unarchive a Vulcan thread. Defaults to your own thread when threadId is omitted.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Update a Vulcan thread", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
