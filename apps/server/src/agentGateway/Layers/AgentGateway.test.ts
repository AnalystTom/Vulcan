...
import { isTemporaryWorktreeBranch } from "@vulcan/shared/git";
  ...
  const gitLayer = Layer.succeed(GitCore, {
    ...
    createDetachedWorktree: (input: { ref: string; path?: string; newBranch?: string }) =>
      Effect.gen(function* () {
        ...
        return {
          worktree: {
            path: input.path ?? "/tmp/worktrees/generated/vulcan",
            ref: input.ref,
...
describe("AgentGateway", () => {
    ...
    return Effect.gen(function* () {
      ...
      const response = yield* harness.postRaw({
        ...
        body: {
          ...
          id: true,
          method: "tools/call",
          params: { name: "vulcan_set_thread_title", arguments: { title: "Must not run" } },
        },
      });
  ...
  it.effect("answers initialize with instructions and lists tools", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.notInclude(String(initResult.instructions), "[Vulcan harness policy");
      ...
      const createThreadProperties = tools.find((tool) => tool.name === "vulcan_create_thread")
      ...
      const createThreadsTool = tools.find((tool) => tool.name === "vulcan_create_threads");
      ...
      const createAutomation = tools.find((tool) => tool.name === "vulcan_create_automation");
      ...
      const updateAutomationMemory = tools.find(
        (tool) => tool.name === "vulcan_update_automation_memory",
      );
      const reportAutomationResult = tools.find(
        (tool) => tool.name === "vulcan_report_automation_result",
      );
      assert.include(
      ...
      const updateAutomationProperties = tools.find(
        (tool) => tool.name === "vulcan_update_automation",
      ...
      extraProjects: [
        {
          ...
          kind: "chat",
          title: "che progetti ci sono in vulcan",
          workspaceRoot: `${homeDir}/Documents/Vulcan/2026-03-01/chat`,
        },
        {
          ...makeProjectShell(),
          ...
          kind: "studio",
          title: "Studio",
          workspaceRoot: `${homeDir}/Documents/Vulcan/Studio`,
        },
        {
          ...makeProjectShell(),
    ...
    return Effect.gen(function* () {
      ...
      const first = toolResultJson(
        (yield* harness.callTool({
          token: "token-parent",
          name: "vulcan_read_thread_activity",
  ...
  it.effect("paginates coalesced message updates as one logical event", () => {
    ...
    return Effect.gen(function* () {
      ...
      const first = toolResultJson(
        (yield* harness.callTool({
          token: "token-parent",
          name: "vulcan_read_thread_events",
  ...
  it.effect("fetches a GitHub pull-request head without creating a local branch", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The worktree is born on a temporary vulcan/* branch, but no branch is
  ...
  it.effect("rejects every destructive tool after the caller turn completes", () => {
    ...
    return Effect.gen(function* () {
      ...
      const attempts = [
        ...
        { name: "vulcan_interrupt_thread", args: { threadId: "thread-child" } },
        {
        ...
        {
          name: "vulcan_create_automation",
          args: { name: "late monitor", prompt: "late" },
        },
        {
          name: "vulcan_cancel_automation",
    ...
    return Effect.gen(function* () {
      ...
      const creationRecaps = harness.dispatched.filter(
        (command) =>
          ...
          command.activity.kind === "vulcan.threads.created",
      );
      ...
      assert.deepEqual(
        ...
        [0, 1].map((index) => ({
          creationSource: "vulcan_mcp" as const,
          sourceThreadId: ThreadId.makeUnsafe("thread-parent"),
          sourceTurnId: TurnId.makeUnsafe("turn-parent-active"),
  ...
  it.effect("accepts memory writes without automationId and via the content alias", () => {
    ...
    return Effect.gen(function* () {
      ...
      const implicit = yield* harness.callTool({
        token: "token-parent",
        name: "vulcan_update_automation_memory",
