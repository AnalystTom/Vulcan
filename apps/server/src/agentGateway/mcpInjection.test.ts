...
describe("agent gateway MCP injection", () => {
  ...
  it("builds a codex config block that references the token env var, not the token", () => {
    ...
    assert.include(block, "[mcp_servers.vulcan]");
    assert.include(block, `url = "${connection.url}"`);
    assert.include(block, `bearer_token_env_var = "${VULCAN_AGENT_GATEWAY_TOKEN_ENV}"`);
  ...
  it("appends the codex section once and keeps existing config intact", () => {
    ...
    assert.include(appended, "[mcp_servers.vulcan]");

    const reappended = appendCodexConfigSection(appended, section);
    assert.equal(reappended.split("[mcp_servers.vulcan]").length, 2);
  });
  ...
  it("merges the token exclusion into a user-defined shell environment policy", () => {
    ...
    const merged = mergeShellEnvPolicyExclude(withExclude, VULCAN_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${VULCAN_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`);

    // Idempotent: the var is not added twice.
    assert.equal(mergeShellEnvPolicyExclude(merged, VULCAN_AGENT_GATEWAY_TOKEN_ENV), merged);

    // A policy table without an exclude key gains one.
    const withoutExclude = ["[shell_environment_policy]", 'inherit = "core"'].join("\n");
    const gained = mergeShellEnvPolicyExclude(withoutExclude, VULCAN_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(gained, `exclude = ["${VULCAN_AGENT_GATEWAY_TOKEN_ENV}"]`);
    assert.include(gained, 'inherit = "core"');

    // No policy table: unchanged (the managed section appends its own).
    assert.equal(
      mergeShellEnvPolicyExclude('[model]\nname = "gpt-5.5"', VULCAN_AGENT_GATEWAY_TOKEN_ENV),
      '[model]\nname = "gpt-5.5"',
    );
  });
  ...
  it("ignores commented and unrelated token references when merging shell exclusions", () => {
    const commentedExample = [
      "[shell_environment_policy]",
      `# exclude = ["${VULCAN_AGENT_GATEWAY_TOKEN_ENV}"]`,
      'exclude = ["AWS_*"]',
    ].join("\n");
    ...
    assert.include(
      ...
      `exclude = ["${VULCAN_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`,
    );

    const unrelatedString = [
      "[shell_environment_policy]",
      `note = "keep ${VULCAN_AGENT_GATEWAY_TOKEN_ENV} private"`,
      'exclude = ["AWS_*"]',
    ].join("\n");
  ...
  it("recognizes only active exact entries in multiline shell exclusion arrays", () => {
    ...
    assert.equal(mergeShellEnvPolicyExclude(existing, VULCAN_AGENT_GATEWAY_TOKEN_ENV), existing);
    ...
    const merged = mergeShellEnvPolicyExclude(tokenOnlyInComment, VULCAN_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${VULCAN_AGENT_GATEWAY_TOKEN_ENV}",`);
  });

  it("detects real TOML table headers, ignoring comments and strings", () => {
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers.vulcan]\nurl = "x"', "[mcp_servers.vulcan]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [mcp_servers.vulcan]  # managed", "[mcp_servers.vulcan]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [ mcp_servers.vulcan ]  # managed", "[mcp_servers.vulcan]"),
    );
    assert.isTrue(configHasTomlTableHeader("  [ mcp_servers . vulcan ]", "[mcp_servers.vulcan]"));
    assert.isTrue(configHasTomlTableHeader('[mcp_servers."vulcan"]', "[mcp_servers.vulcan]"));
    assert.isTrue(configHasTomlTableHeader("['mcp_servers'.'vulcan']", "[mcp_servers.vulcan]"));
    assert.isTrue(configHasTomlTableHeader('[mcp_servers."vulc\\u0061n"]', "[mcp_servers.vulcan]"));
    assert.isTrue(
      configHasTomlTableHeader('["shell_environment_policy"]', "[shell_environment_policy]"),
    );
    assert.isFalse(configHasTomlTableHeader('["mcp_servers.vulcan"]', "[mcp_servers.vulcan]"));
    assert.isFalse(configHasTomlTableHeader('[mcp_servers."vulc\\qan"]', "[mcp_servers.vulcan]"));
    // A commented-out example block must not count as the table being present.
    assert.isFalse(configHasTomlTableHeader("# [mcp_servers.vulcan]", "[mcp_servers.vulcan]"));
    assert.isFalse(
      configHasTomlTableHeader('note = "see [mcp_servers.vulcan] docs"', "[mcp_servers.vulcan]"),
    );
  });
        ...
        result:
          request.method === "tools/list"
            ? {
                tools: [
                  {
                    name: "vulcan_list_threads",
                    description: "List Vulcan threads.",
  ...
  it("uses the ACP http transport when the agent advertises support", () => {
    const servers = buildAcpVulcanMcpServers({
      connection,
    ...
    assert.deepEqual(servers, [
      {
        ...
        args: stdioProxy.args,
        env: [
          { name: "VULCAN_AGENT_GATEWAY_URL", value: connection.url },
          { name: VULCAN_AGENT_GATEWAY_TOKEN_ENV, value: connection.bearerToken },
        ],
      },
    ]);
  });
