...
export const MODEL_SELECTION_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    model: {
      type: "string",
      description: "Exact model slug from vulcan_capabilities providers[].models[].slug.",
    },
    options: {
      type: "object",
      description: AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION,
...
export function decodeCreateThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(VulcanCreateThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Vulcan creation plan: ${errorText(error)}`);
  }
}

export function decodeWaitForThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(VulcanWaitForThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Vulcan wait request: ${errorText(error)}`);
  }
}
