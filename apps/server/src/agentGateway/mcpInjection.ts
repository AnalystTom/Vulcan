/**
 * Provider-facing config builders for the Vulcan agent gateway.
 *
 ...
 *
 * - Codex: `[mcp_servers.vulcan]` TOML block (streamable HTTP +
...
export const VULCAN_MCP_SERVER_NAME = "vulcan";
export const VULCAN_AGENT_GATEWAY_TOKEN_ENV = "VULCAN_AGENT_GATEWAY_TOKEN";
export const VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV = "VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN";
export const VULCAN_AGENT_GATEWAY_URL_ENV = "VULCAN_AGENT_GATEWAY_URL";
 ...
 * the block references an env var that Vulcan sets per app-server process.
 *
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    `[mcp_servers.${VULCAN_MCP_SERVER_NAME}]`,
    `url = ${JSON.stringify(endpointUrl)}`,
    `bearer_token_env_var = ${JSON.stringify(VULCAN_AGENT_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(VULCAN_AGENT_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
 ...
 * provider process dedicated to the owning Vulcan thread or an exclusive
...
async function postAgentGatewayJsonRpc(input: {
  ...
  if (!response.ok) {
    throw new Error(`Vulcan MCP request failed with HTTP ${String(response.status)}.`);
  }
  const payload: unknown = await response.json();
  if (!isRecord(payload) || payload.jsonrpc !== "2.0") {
    throw new Error("Vulcan MCP returned an invalid JSON-RPC response.");
  }
  if ("error" in payload) {
    const failure = isRecord(payload.error) ? payload.error : null;
    throw new Error(failure?.message ? String(failure.message) : "Vulcan MCP request failed.");
  }
  if (payload.id !== id || !("result" in payload)) {
    throw new Error("Vulcan MCP returned a mismatched JSON-RPC response.");
  }
...
export async function listAgentGatewayMcpTools(input: {
  ...
  if (!isRecord(result) || !Array.isArray(result.tools)) {
    throw new Error("Vulcan MCP tools/list returned an invalid tool catalog.");
  }
  return result.tools.map((value) => {
    if (
      ...
      throw new Error("Vulcan MCP tools/list returned an invalid tool descriptor.");
    }
    return {
      name: value.name,
...
/**
 * Build the secret-free MCP fragment installed with Vulcan's Antigravity
  ...
  return {
    mcpServers: {
      [VULCAN_MCP_SERVER_NAME]: {
        ...
        env: {
          [VULCAN_AGENT_GATEWAY_URL_ENV]: `$${VULCAN_AGENT_GATEWAY_URL_ENV}`,
          [VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${VULCAN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
 ...
 */
export function buildAcpVulcanMcpServers(input: {
  readonly connection: AgentGatewayMcpConnection;
  ...
  return [
    {
      ...
      env: [
        { name: VULCAN_AGENT_GATEWAY_URL_ENV, value: input.connection.url },
        { name: VULCAN_AGENT_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
