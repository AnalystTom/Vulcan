...
import { createCachedLoginShellPathReader } from "@vulcan/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveVulcanHomeDirectory,
} from "@vulcan/shared/vulcanHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the Vulcan home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveVulcanHomeDirectory({ configuredHome: raw }));
