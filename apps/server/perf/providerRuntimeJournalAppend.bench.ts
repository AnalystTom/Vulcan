...
const EVENT_COUNT = readPositiveInteger("VULCAN_BENCH_EVENTS", 10_000);
const SAMPLE_COUNT = readPositiveInteger("VULCAN_BENCH_SAMPLES", 8);
...
function createDatabase(): { readonly database: Database; readonly dispose: () => void } {
  const directory = mkdtempSync(path.join(os.tmpdir(), "vulcan-runtime-journal-bench-"));
