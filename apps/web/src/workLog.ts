...
} from "@vulcan/shared/subagents";
import {
  approvalRequestKindFromRequestType,
  type ApprovalRequestKind,
} from "@vulcan/shared/threadSummary";
import { summarizeToolRawOutput } from "@vulcan/shared/toolOutputSummary";
...
export interface WorkLogEntry {
  ...
  toolName?: string;
  toolCallId?: string;
  toolStatus?: VulcanMcpToolStatus;
  ...
  vulcanThreadCreation?: WorkLogVulcanThreadCreation;
...
export interface WorkLogVulcanCreatedThread {
  threadId: string;
  title: string;
...
export interface WorkLogVulcanThreadCreation {
  operationId: string;
  requestedCount: number;
  createdCount: number;
  threads: ReadonlyArray<WorkLogVulcanCreatedThread>;
}
...
function extractWorkLogVulcanThreadCreation(
  payload: Record<string, unknown> | null,
): WorkLogVulcanThreadCreation | null {
  if (!payload) {
    return null;
  }
  ...
  const threads = rawThreads.flatMap((value): WorkLogVulcanCreatedThread[] => {
...
function toDerivedWorkLogEntry(activity: OrchestrationThreadActivity): DerivedWorkLogEntry {
  ...
  if (activity.kind === "vulcan.threads.created") {
    const vulcanThreadCreation = extractWorkLogVulcanThreadCreation(payload);
    if (vulcanThreadCreation) {
      entry.vulcanThreadCreation = vulcanThreadCreation;
    }
  }
  const readableTitle =
...
function mergeDerivedWorkLogEntries(
  ...
  const vulcanThreadCreation = next.vulcanThreadCreation ?? previous.vulcanThreadCreation;
  ...
  return {
    ...
    ...(vulcanThreadCreation ? { vulcanThreadCreation } : {}),
