...
// Purpose: Imports Vulcan browser state before renderer stores hydrate after a desktop origin move.
...
export function importVulcanStorageSnapshot(
  snapshot: VulcanStorageSnapshot | null,
...
export function bootstrapVulcanStorageOriginMigration(): void {
  const bridge = globalThis.window?.desktopBridge?.storageMigration;
  if (!bridge) return;

  try {
    const snapshot = bridge.readSnapshot();
    if (snapshot && importVulcanStorageSnapshot(snapshot)) {
...
bootstrapVulcanStorageOriginMigration();
