import type { GitBranch, ProviderKind } from "@vulcan/contracts";
import {
...
} from "@vulcan/shared/composerSlashCommands";
...
> = {
  ...
  feedback: {
    command: "feedback",
    label: "/feedback",
    description: "Send feedback to the Vulcan team",
    source: "app",
  },
  automation: {
    command: "automation",
  ...
  const availableCommands: ComposerSlashCommand[] =
    input.provider !== "claudeAgent"
      ...
      : [
          ...
          // creates a Vulcan split/context clone before the provider sees the first turn.
          // /export is app-level too — Vulcan owns the thread transcript, so the download
