import { MessageId, TurnId, type OrchestrationThreadActivity } from "@vulcan/contracts";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps created-automation milestones and exposes their card fields despite a null turn id", () => {
    const activities: OrchestrationThreadActivity[] = [
      ...
      makeActivity({
        ...
        summary: "Created automation: Watch Vulcan PR 231 - Every 5m",
        tone: "info",
        payload: {
          source: "chat-composer",
          automationId: "automation-7",
          automationName: "Watch Vulcan PR 231",
  ...
  it("exposes a provider-independent Vulcan thread creation recap", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "vulcan-created-threads",
        createdAt: "2026-02-23T00:00:05.000Z",
        turnId: "turn-1",
        kind: "vulcan.threads.created",
        summary: "Created 2 Vulcan threads",
        tone: "info",
        payload: {
    ...
    expect(entry?.vulcanThreadCreation).toEqual({
      operationId: "gateway:create:two-workers",
      requestedCount: 2,
      createdCount: 2,
      threads: [
  ...
  it("hides repeated non-adjacent recovery rows for the same provider turn", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        summary: "Vulcan recovered a stale running state",
        turnId: "turn-stale",
              ...
              commandActions: [
                {
                  ...
                  name: "README.md",
                  path: "/Users/emanueledipietro/Developer/Testing/vulcan/README.md",
                },
              ],
            },
          },
        ...
        payload: {
          ...
          data: {
            item: {
              ...
              command: "/bin/zsh -lc 'git status --short'",
              cwd: "/Users/emanueledipietro/Developer/Testing/vulcan",
              status: "completed",
              commandActions: [{ type: "unknown", command: "git status --short" }],
  ...
  it("preserves cancellation when an owning turn aborts", () => {
    const turnId = TurnId.makeUnsafe("turn-with-cancelled-vulcan-tool");
    const entries = deriveWorkLogEntries(
      [
        makeActivity({
          id: "cancelled-vulcan-start",
          createdAt: "2026-02-23T00:00:01.000Z",
          kind: "tool.started",
          summary: "Vulcan create thread",
          turnId,
          payload: {
            itemType: "mcp_tool_call",
            title: "Vulcan create thread",
            data: {
              toolCallId: "cancelled-vulcan-call",
      ...
      [
        makeActivity({
          ...
          payload: {
            ...
            status: "interrupted",
            data: {
              toolCallId: "interrupted-vulcan-call",
              toolName: "mcp__vulcan__vulcan_create_thread",
            },
          },
        }),
  ...
  it("presents Vulcan MCP activity consistently across provider item shapes", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "vulcan-mcp-create-thread-progress",
        kind: "tool.updated",
        ...
        payload: {
          ...
          title: "MCP tool call",
          data: {
            toolCallId: "vulcan-mcp-create",
            toolName: "mcp__vulcan__vulcan_create_thread",
          },
        },
      }),
      makeActivity({
        id: "vulcan-dynamic-send-message-progress",
        kind: "tool.updated",
        summary: "Tool call",
        payload: {
          itemType: "dynamic_tool_call",
          title: "Vulcan__vulcan_send_message",
          data: {
            toolCallId: "vulcan-dynamic-send",
          },
        },
      }),
      makeActivity({
        id: "vulcan-file-change-list-threads-progress",
        kind: "tool.updated",
        summary: "File change",
        payload: {
          itemType: "file_change",
          title: "mcp__Vulcan__vulcan_list_threads",
          data: {
            toolCallId: "vulcan-file-change-list",
          },
        },
      }),
    ];
    ...
    expect(entries.map((entry) => [entry.itemType, entry.toolTitle])).toEqual(
      expect.arrayContaining([
        ["mcp_tool_call", "Vulcan is creating a thread"],
        ["dynamic_tool_call", "Vulcan is sending a message"],
        ["file_change", "Vulcan is listing threads"],
      ]),
    );
    expect(entries).toHaveLength(3);
  });

  it("preserves a failed Vulcan MCP result as a failed activity sentence", () => {
    const [entry] = deriveWorkLogEntries(
      [
        makeActivity({
          id: "vulcan-create-threads-failed",
          kind: "tool.completed",
          summary: "vulcan__vulcan_create_threads",
          payload: {
            itemType: "mcp_tool_call",
            status: "failed",
            data: {
              toolCallId: "vulcan-create-failed",
    ...
    expect(entry).toMatchObject({
      toolStatus: "failed",
      toolTitle: "Vulcan couldn't create threads",
