...
describe("store facade", () => {
  ...
  it("preserves optimistic createBranchFlowCompleted during stale read-model syncs", () => {
    ...
    const optimisticState = setThreadWorkspace(
      makeState(
        makeThread({
          envMode: "worktree",
          branch: "vulcan/tmp-working",
          worktreePath: "/tmp/project/.worktrees/tmp-working",
          associatedWorktreePath: "/tmp/project/.worktrees/tmp-working",
          associatedWorktreeBranch: "vulcan/tmp-working",
          associatedWorktreeRef: "vulcan/tmp-working",
        }),
      ),
      threadId,
      {
  ...
  it("renames a project locally without changing its remote or folder names", () => {
    ...
    const next = renameProjectLocally(state, ProjectId.makeUnsafe("project-1"), "vulcan");
  ...
  it("persists project aliases immediately when the local alias changes", async () => {
    ...
    try {
      ...
      freshStore.useStore.getState().renameProjectLocally(projectId, "vulcan");

      expect(setItem).toHaveBeenCalled();
      expect(JSON.parse(storage.get("vulcan:renderer-state:v8") ?? "{}")).toMatchObject({
        projectNamesByCwd: {
          "/tmp/project": "vulcan",
        },
      });
