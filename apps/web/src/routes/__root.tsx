...
import { defaultTerminalTitleForCliKind } from "@vulcan/shared/terminalThreads";
import { isThreadDetailEventFor } from "@vulcan/shared/threadDetailEvents";
import {
  Outlet,
...
function TransportCompatibilityView({ issue }: { issue: WsCompatibilityError }) {
  const title =
    issue.action === "update-client"
      ? "This Vulcan client needs an update."
      : issue.action === "update-server"
        ? "The Vulcan server needs an update."
        : "Vulcan needs to reconnect with a matching build.";
  const guidance =
    issue.action === "update-client"
      ? "Update or reload this client, then reconnect."
      : issue.action === "update-server"
        ? "Update or restart the server, then reload this client."
        : "Reload the app. If this repeats, restart Vulcan so the client and server use matching builds.";
