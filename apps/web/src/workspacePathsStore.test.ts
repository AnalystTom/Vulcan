...
describe("workspacePathsStore", () => {
  ...
  it("keeps chat workspace root while server config is still loading", async () => {
    ...
    useWorkspacePathsStore.getState().setChatWorkspaceRoot("/Users/tester/Documents/Vulcan");
  ...
  it("keeps studio workspace root while server config is still loading", async () => {
    ...
    useWorkspacePathsStore
      .getState()
      .setStudioWorkspaceRoot("/Users/tester/Documents/Vulcan/Studio");
    ...
    expect(useWorkspacePathsStore.getState().studioWorkspaceRoot).toBe(
      "/Users/tester/Documents/Vulcan/Studio",
    );
  });
  ...
  it("migrates cached paths without retaining legacy workspace pages", async () => {
    ...
    localStorage.setItem(
      "vulcan:workspace-pages:v2",
      JSON.stringify({
        state: {
          homeDir: "/Users/legacy",
          chatWorkspaceRoot: "/Users/legacy/Documents/Vulcan",
    ...
    expect(localStorage.getItem("vulcan:workspace-pages:v2")).toBeNull();
    expect(localStorage.getItem("vulcan:workspace-paths:v1")).not.toBeNull();
  });
});
