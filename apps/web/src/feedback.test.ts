...
describe("formatFeedbackSummary", () => {
  it("opens in the reporter's voice and lists the diagnostics a maintainer needs", () => {
    ...
    expect(summary).toBe(
      [
        "I ran into a bug in Vulcan 0.5.1, using codex with gpt-5.6-sol.",
        "",
        "Report type: Bug",
        "App version: 0.5.1",
        "Provider: codex",
  ...
  it("falls back to a neutral opening and omits fields the session never set", () => {
    ...
    expect(summary).toContain(
      "I have some feedback in Vulcan 0.5.1, using codex with gpt-5.6-sol.",
    );
  ...
  it.each(FEEDBACK_CATEGORIES)(
    ...
    ({ value, label, lead }) => {
      ...
      expect(summary.startsWith(`${lead} in Vulcan 0.5.1`)).toBe(true);
  ...
  it("describes feedback sent outside an active chat without inventing provider context", () => {
    ...
    expect(summary).toContain("I have some feedback in Vulcan 0.5.1 outside an active chat.");
