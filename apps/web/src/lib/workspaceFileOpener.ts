...
import { isSupportedLocalPreviewFilePath } from "@vulcan/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
  isWorkspaceRelativePathSafe,
  workspaceRelativePathOf,
} from "@vulcan/shared/path";
import { isScratchWorkspacePath } from "@vulcan/shared/threadWorkspace";
...
const VULCAN_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const VULCAN_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveVulcanPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!VULCAN_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${VULCAN_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveVulcanPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
