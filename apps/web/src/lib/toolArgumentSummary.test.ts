...
describe("isPrefixedToolArgumentSummary", () => {
  it("recognizes `ToolName: {json}` and `ToolName: [json]` details", () => {
    expect(
      isPrefixedToolArgumentSummary('mcp__vulcan__vulcan_read_thread: {"threadId":"c357"}'),
    ).toBe(true);
...
describe("toolArgumentSummaryToolName", () => {
  it("extracts the identifier prefix whether or not JSON args follow", () => {
    ...
    expect(toolArgumentSummaryToolName('mcp__vulcan__vulcan_read_thread: {"threadId":"c3"}')).toBe(
      "mcp__vulcan__vulcan_read_thread",
    );
  });
