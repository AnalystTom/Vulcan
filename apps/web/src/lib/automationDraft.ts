...
import { DEFAULT_AUTOMATION_FAST_INTERVAL_MAX_ITERATIONS } from "@vulcan/contracts";
import type {
  AutomationMode,
  AutomationSchedule,
...
export function buildAutomationDraftWarnings(input: {
    ...
    warnings.push({
      ...
      detail:
        input.worktreeMode === "auto"
          ? "If Vulcan cannot create a worktree, runs may fall back to editing the active project checkout."
  ...
  if (
    ...
    warnings.push({
      ...
      detail: "Vulcan was not fully confident about the parsed automation fields.",
      requiresAcknowledgement: false,
    });
  }
