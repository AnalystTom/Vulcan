...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.vulcan/worktrees/vulcan-mvp/vulcan-4e609bb8",
    );
    expect(result).toBe("vulcan-4e609bb8");
  });

  it("keeps legacy .vulcan worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.vulcan/worktrees/vulcan-mvp/vulcan-legacy123",
    );
    expect(result).toBe("vulcan-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.vulcan\\worktrees\\vulcan-mvp\\vulcan-4e609bb8",
    );
    expect(result).toBe("vulcan-4e609bb8");
  });

  it("uses the final segment even when outside ~/.vulcan/worktrees", () => {
