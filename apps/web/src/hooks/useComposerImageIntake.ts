...
import { PROVIDER_SEND_TURN_MAX_ATTACHMENTS, type ThreadId } from "@vulcan/contracts";
...
export class ComposerImageIntakeQueue {
  ...
  enqueue(job: ComposerImageIntakeJob): Promise<void> {
    ...
    const task = this.#tail
      ...
      .catch((cause) => {
        if (this.#isStale(generation)) return;
        job.onError(cause instanceof Error ? cause.message : "Vulcan could not prepare image.");
      })
