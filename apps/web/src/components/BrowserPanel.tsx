...
import { localServerPrimaryLabel } from "@vulcan/shared/localServers";
import { resolveDesktopDipRectFromCssRect } from "@vulcan/shared/desktopChrome";
import {
  BROWSER_BLANK_URL,
  isBlankBrowserTabUrl,
  resolveCopyableBrowserTabUrl,
} from "@vulcan/shared/browserSession";
import {
  BROWSER_COPY_LINK_TOAST_TITLE,
  isBrowserCopyLinkChord,
} from "@vulcan/shared/browserShortcuts";
...
const BROWSER_WEBVIEW_PARTITION = "persist:vulcan-browser";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("vulcan:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${VULCAN_BROWSER_LABEL} panel perf]`, {
        threadId,
