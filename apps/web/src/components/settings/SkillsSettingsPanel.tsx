...
// catalog (~/.vulcan/skills plus each provider's skills folder), shows which provider
...
import type { ProviderKind, ServerSettings } from "@vulcan/contracts";
...
export function SkillsSettingsPanel() {
  ...
  const vulcanSkillsDir = catalogQuery.data?.vulcanSkillsDir;

  return (
    <div className="space-y-8">
      <SettingsSection title="Portable skills">
        <SettingsRow
          title="Vulcan skills folder"
          description="Skills placed here are available on every provider. When a provider already ships its own copy of a skill, that copy is used; otherwise Vulcan's copy is the fallback."
          status={
            vulcanSkillsDir ? (
              <code className="break-all text-[11px] text-muted-foreground">{vulcanSkillsDir}</code>
            ) : null
          }
          control={
      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Vulcan could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Vulcan skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
