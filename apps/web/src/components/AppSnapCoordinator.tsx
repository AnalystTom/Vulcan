...
// Purpose: Routes native macOS AppSnaps into the correct Vulcan composer draft.
    ...
    async (capture: DesktopAppSnapCapture) => {
      ...
      if (resolvedTarget.kind === "existing") {
        ...
        if (result.threadId) {
          ...
          const focused = focusedTargetRef.current;
          if (!focused) throw new Error("Vulcan could not create a task for this AppSnap.");
          target = focused;
          openChatThreadPage(target.threadId);
        }
      }
      ...
      if (!image) throw new Error(error ?? "Vulcan could not attach the captured AppSnap.");
      ...
      toastManager.add({
        ...
        description:
          persistenceResult === "unverified"
            ? "The capture is attached, but Vulcan could not verify its draft metadata. If it is missing after a reload, Vulcan will attach it again."
