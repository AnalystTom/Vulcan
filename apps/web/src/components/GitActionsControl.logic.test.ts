import type { GitStatusResult } from "@vulcan/contracts";
...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Vulcan as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "vulcan/fix-toast-copy");
  });

  it("normalizes an existing legacy vulcan namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "vulcan/refine-toolbar-actions");
    assert.equal(branch, "vulcan/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Vulcan", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "vulcan/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "vulcan/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "vulcan/deadbeef";
