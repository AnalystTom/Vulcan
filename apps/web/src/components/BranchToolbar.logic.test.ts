import type { GitBranch } from "@vulcan/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.vulcan/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.vulcan/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .vulcan worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.vulcan/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/vulcan-pi",
        existingAssociatedWorktreeBranch: "vulcan/pi",
        existingAssociatedWorktreeRef: "vulcan/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/vulcan-pi",
      associatedWorktreeBranch: "vulcan/pi",
      associatedWorktreeRef: "vulcan/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.vulcan/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.vulcan/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.vulcan/worktrees/feature-b",
      nextWorktreePath: "/repo/.vulcan/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.vulcan/worktrees/feature-a",
      nextWorktreePath: "/repo/.vulcan/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
