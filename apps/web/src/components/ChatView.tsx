...
import { automationRequiresTargetThread } from "@vulcan/shared/automationMode";
import { respondingInteractionReclaimAt } from "@vulcan/shared/pendingInteractions";
import { providerSupportsNativeTurnSteering } from "@vulcan/shared/providerMetadata";
import { getModelCapabilities, normalizeModelSlug } from "@vulcan/shared/model";
import {
  resolveLatestTailUserMessageEditTarget,
  resolveTailUserMessageEditTarget,
} from "@vulcan/shared/conversationEdit";
import { threadExportBlockedReason } from "@vulcan/shared/threadExport";
import { pendingRequestInstanceKey } from "@vulcan/shared/threadSummary";
import {
  buildPromptThreadTitleFallback,
  GENERIC_CHAT_THREAD_TITLE,
} from "@vulcan/shared/chatThreads";
import {
  resolveThreadWorkspaceState,
  resolveThreadBranchSourceCwd,
  resolveThreadWorkspaceCwd as resolveSharedThreadWorkspaceCwd,
} from "@vulcan/shared/threadEnvironment";
import {
...
// back-and-forth (user request, Vulcan's clarifying questions) inline like Codex.
        ...
        if (serverThread) {
          ...
          if (!api) {
            toastManager.add({
              ...
              description: "Vulcan is not connected to the server.",
            });
            return false;
          }
    ...
    (style: ThreadMarkerStyle, color: ThreadMarkerColor) => {
      ...
      if (!range) {
        toastManager.add({
          ...
          description: "Try including a few more words so Vulcan can find the exact place.",
        });
        return;
      }
      dismissTranscriptSelectionAction();
      ...
      return await (async () => {
          ...
          void (async () => {
            try {
              ...
              toastManager.add({
                ...
                description:
                  "The automation was created, but Vulcan could not add the activity note.",
              });
            }
          })();
        }
        ...
        .catch((error: unknown) => {
          toastManager.add({
            ...
            description:
              error instanceof Error ? error.message : "Vulcan could not save the automation.",
          });
          return false;
        })
        .finally(() => {
      ...
      const promoteDraftForAutomation = async (): Promise<ThreadId | null> => {
        ...
        if (result === "unavailable") {
          toastManager.add({
            ...
            description: "Vulcan could not promote this draft before saving the automation.",
          });
          return null;
        }
      ...
      try {
        ...
        toastManager.add({
          ...
          description:
            error instanceof Error
              ? error.message
              : "Vulcan could not promote this draft before saving the automation.",
        });
        return null;
      }
    },
            ...
            {shouldRenderChatPaneContent && isCenteredEmptyLanding ? (
              ...
              >
                ...
                <div className="flex w-full flex-col justify-center">
                  ...
                  >
                    <VulcanLogo aria-label="Vulcan logo" className="size-10" />
                    <h2
