...
import { isGenericChatThreadTitle } from "@vulcan/shared/chatThreads";
import { getDefaultModel } from "@vulcan/shared/model";
import { pluralize } from "@vulcan/shared/text";
...
type DebugFeatureFlagsWindow = Window & {
  vulcanShowFeatureFlags?: () => void;
  vulcanHideFeatureFlags?: () => void;
};
...
const VULCAN_DOCS_URL = "https://github.com/AnalystTom/Agent_panel/docs";
      ...
      <Menu>
        ...
        <ComposerPickerMenuPopup align="end" side="top" className="w-64 min-w-64">
          ...
          <MenuGroup>
            ...
            <MenuItem
              className={SIDEBAR_CONTEXT_MENU_ITEM_CLASS_NAME}
              onClick={() => openExternalLink(VULCAN_DOCS_URL)}
            >
 ...
 */
const ACTIVITY_ONBOARDING_STORAGE_KEY = "vulcan:activity-onboarding:v1";
...
const SIDEBAR_SURFACE_PICKER_COPY: Record<SidebarView, { title: string; description: string }> = {
  threads: { title: "Vulcan", description: "Build, debug, and ship" },
  bots: { title: "Bots", description: "Your standing team of agents" },
...
export default function Sidebar() {
  ...
  const routeSurface = resolveSidebarSurface(pathname);
  ...
  const botsSectionVisible = appSettings.showBotsSection;
  ...
  useEffect(() => {
    ...
    debugWindow.vulcanShowFeatureFlags = showFeatureFlags;
    debugWindow.vulcanHideFeatureFlags = hideFeatureFlags;
    window.addEventListener("storage", updateVisibility);
    updateVisibility();

    return () => {
      window.removeEventListener("storage", updateVisibility);
      if (debugWindow.vulcanShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.vulcanShowFeatureFlags;
      }
      if (debugWindow.vulcanHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.vulcanHideFeatureFlags;
      }
    };
  }, []);
  ...
  const activeRouteProject = activeRouteProjectId
    ...
    : null;
  // An active Studio thread pulls the sidebar onto the Studio surface even from a thread route.
  ...
  const surface: SidebarSurface =
    ...
    isStudioContainerProject(activeRouteProject, {
      ...
      ? "studio"
      : routeSurface;
  const isOnStudio = surface === "studio";
  const isOnBots = surface === "bots";
  const isOnSettings = surface === "settings";
  const botRoster = useBots();
  ...
  const pinnedThreads = useMemo(
    () =>
      getPinnedThreadsForSidebar(
        surface === "studio" ? studioSidebarTreeThreads : activeSpaceNonStudioSidebarTreeThreads,
        pinnedThreadIds,
      ),
    [activeSpaceNonStudioSidebarTreeThreads, pinnedThreadIds, studioSidebarTreeThreads, surface],
  );
  ...
  useEffect(() => {
    ...
    lastActiveSidebarSegmentRef.current = surface === "studio" ? "studio" : "threads";
  }, [surface]);
  ...
  useEffect(() => {
    ...
    if (surface === "studio" && !studioSectionVisible) {
      handleSidebarViewChange("threads");
      return;
    }
    if (surface === "bots" && !botsSectionVisible) {
      handleSidebarViewChange("threads");
    }
  }, [botsSectionVisible, handleSidebarViewChange, studioSectionVisible, surface]);
  ...
  >(() => {
    ...
    // Projects-side store change. Mirrors the surface gate on
    // studioChatThreadRows.
    if (surface !== "studio") {
  ...
  const surfaceProjects = surface === "studio" ? studioProjects : standardProjects;
  const surfaceProjectSidebarDataById =
    surface === "studio" ? studioProjectSidebarDataById : standardProjectSidebarDataById;
  ...
  useEffect(() => {
    if (surface === "settings" || routeThreadId === null) {
      return;
    }
    ...
    return () => window.clearTimeout(settle);
  }, [surface, routeSearch.splitViewId, routeThreadId]);
  ...
  // "Vulcan" for project-less chats) with the hover cards and Activity rows.
  ...
  function renderProjectItem(
    ...
    // A project reads as "running" when Vulcan tracks a run for it or when a
    // local server (possibly started outside Vulcan) is attributed by cwd.
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (command === "space.previous" || command === "space.next") {
        if (!isProjectsSidebarSurface(surface)) return;
        event.preventDefault();
        event.stopPropagation();
  ...
  const searchPaletteActions = useMemo<SidebarSearchAction[]>(
    () => [
      ...
      {
        id: "feedback",
        label: "Feedback Vulcan",
        description: "Send feedback or report an issue to the Vulcan team.",
        keywords: ["feedback", "bug", "issue", "problem", "report", "support", "vulcan"],
      },
      {
        id: "settings",
        label: "Settings",
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              ...
              description: `Vulcan is preparing version ${nextState.availableVersion ?? "available"} in the background.`,
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              type: "info",
              title: "Preparing update",
              description: "Vulcan is downloading the update in the background.",
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `Vulcan ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
          ...
          <>
            <div className="flex items-center gap-1 pt-0 pb-1 pr-2.5 pl-1.5">
              <SidebarSurfacePicker
                views={[
                  "threads",
                  ...(botsSectionVisible ? (["bots"] as const) : []),
                  ...(studioSectionVisible ? (["studio"] as const) : []),
                ]}
                activeView={isOnBots ? "bots" : isOnStudio ? "studio" : "threads"}
