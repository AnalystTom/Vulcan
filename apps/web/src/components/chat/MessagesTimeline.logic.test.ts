import { CheckpointRef, MessageId, OrchestrationProposedPlanId, TurnId } from "@vulcan/contracts";
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("preserves Vulcan tool calls when a separate creation recap is present", () => {
    const createTool = workEntry(
      "vulcan-create-tool",
      "2026-01-01T00:00:01Z",
      "Vulcan created threads",
    );
    ...
    expect(collapsedSignature(messageRow(rows, "a1")!)).toEqual([
      "work:vulcan-create-tool",
      "work:vulcan-create-recap",
    ]);
  });
