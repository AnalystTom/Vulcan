...
import { CheckpointRef, MessageId, ThreadId, TurnId } from "@vulcan/contracts";
...
describe("MessagesTimeline", () => {
  ...
  it("labels only the first message when another task created the conversation", async () => {
    ...
    expect(markup).toContain("Sent by Vulcan from another thread");
    expect(markup).toContain('aria-label="Open source thread"');
    expect(markup.indexOf("Sent by Vulcan from another thread")).toBeLessThan(
  ...
  it("shows the Vulcan mark for every provider-specific tool row shape", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    ...
    const claudeMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-vulcan-claude",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-vulcan-claude",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "dynamic_tool_call",
              toolTitle: "Vulcan__vulcan_create_thread",
              toolName: "Vulcan__vulcan_create_thread",
              detail: "Vulcan__vulcan_create_thread",
    ...
    expect(claudeMarkup).toContain('data-tool-icon="vulcan"');
    expect(claudeMarkup).not.toContain('data-tool-icon="mcp"');
    expect(claudeMarkup).toContain("Vulcan is creating a thread");
    expect(claudeMarkup).not.toContain("Vulcan__vulcan_create_thread");
    ...
    const codexMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-vulcan-codex",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-vulcan-codex",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "file_change",
              toolTitle: "mcp__Vulcan__vulcan_list_threads",
              detail: "mcp__Vulcan__vulcan_list_threads",
            },
          },
        ]}
      />,
    );
    expect(codexMarkup).toContain('data-tool-icon="vulcan"');
    expect(codexMarkup).toContain("Vulcan listed threads");
    expect(codexMarkup).not.toContain("mcp__Vulcan__vulcan_list_threads");
    ...
    const failedMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-vulcan-failed",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-vulcan-failed",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
              toolName: "mcp__vulcan__vulcan_create_threads",
              toolStatus: "failed",
    ...
    expect(failedMarkup).toContain("Vulcan couldn&#x27;t create threads");
    expect(failedMarkup).toContain("Claude rejected reasoningEffort");
  });

  // Browser calls get the globe rather than the generic Vulcan mark: a browsing
  // row is about a page, and the surface it acted on is the first thing to read.
  it("uses the browser icon and action name for Vulcan browser calls", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...makeTimelineBaseProps()}
        timelineEntries={[
          {
            id: "entry-inline-vulcan-browser",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-vulcan-browser",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
    ...
    expect(markup).not.toContain("Vulcan: Browser Open");
  ...
  it("hides raw `ToolName: {json}` argument details behind the humanized heading", async () => {
    ...
    const readThreadMarkup = renderSingleToolRow({
      id: "work-vulcan-read-thread-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__vulcan__vulcan_read_thread",
      detail: 'mcp__vulcan__vulcan_read_thread: {"threadId":"c357d8c5-b4c1-47d0"}',
      activityKind: "tool.completed",
    });
    expect(readThreadMarkup).toContain("Vulcan read a thread");
    expect(readThreadMarkup).not.toContain("mcp__vulcan__vulcan_read_thread:");
    expect(readThreadMarkup).not.toContain("threadId");

    const diagnoseMarkup = renderSingleToolRow({
      id: "work-vulcan-diagnose-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__vulcan__vulcan_diagnose_thread",
      detail: 'mcp__vulcan__vulcan_diagnose_thread: {"threadId":"09a1615d-084f-40b9"}',
      activityKind: "tool.completed",
    });
    expect(diagnoseMarkup).toContain("Vulcan diagnosed a thread");
    expect(diagnoseMarkup).not.toContain("mcp__vulcan__vulcan_diagnose_thread:");
    ...
    const dynamicToolMarkup = renderSingleToolRow({
      ...
      detail: 'ToolSearch: {"query":"select:mcp__vulcan__vulcan_read_thread_events"}',
      activityKind: "tool.completed",
    });
    ...
    expect(failedArgsMarkup).toContain("Vulcan couldn&#x27;t create threads");
    expect(failedArgsMarkup).toContain("Invalid params");
  });

  it("keeps Vulcan tool calls and adds a thread creation recap at the end of the turn", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const assistantMessageId = MessageId.makeUnsafe("message-vulcan-recap");
    const workEntries = [
      {
        id: "entry-vulcan-create-tool",
        kind: "work",
        createdAt: "2026-03-17T19:12:28.000Z",
        entry: {
          id: "work-vulcan-create-tool",
          createdAt: "2026-03-17T19:12:28.000Z",
          label: "MCP tool call",
          tone: "tool",
          itemType: "mcp_tool_call",
          toolName: "mcp__vulcan__vulcan_create_threads",
          toolTitle: "Vulcan created threads",
          activityKind: "tool.completed",
        },
      },
      {
        id: "entry-vulcan-create-recap",
        kind: "work",
        createdAt: "2026-03-17T19:12:29.000Z",
        entry: {
          id: "work-vulcan-create-recap",
          createdAt: "2026-03-17T19:12:29.000Z",
          label: "Created 2 Vulcan threads",
          tone: "info",
          activityKind: "vulcan.threads.created",
          vulcanThreadCreation: {
    ...
    expect(liveMarkup).toContain("Vulcan created threads");
    expect(liveMarkup).not.toContain('data-vulcan-thread-creation-card="true"');
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          ...workEntries,
          {
            id: "entry-vulcan-recap-assistant",
            kind: "message",
            createdAt: "2026-03-17T19:12:30.000Z",
            message: {
              id: assistantMessageId,
    ...
    expect(markup).toContain("Worked for");
    expect(markup).toContain('data-vulcan-thread-creation-card="true"');
    ...
    expect(markup.indexOf("Both threads are running.")).toBeLessThan(
      markup.indexOf('data-vulcan-thread-creation-card="true"'),
    );
  });
          ...
          new Map([
            [
              assistantMessageId,
              {
                ...
                checkpointRef: CheckpointRef.makeUnsafe("refs/vulcan/checkpoints/thread/turn/1"),
                status: "ready",
                completedAt: "2026-03-17T19:12:30.000Z",
                assistantMessageId,
                files: [
