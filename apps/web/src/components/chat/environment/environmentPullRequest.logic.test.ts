...
import type { GitPullRequestComment, PullRequestComment } from "@vulcan/contracts";
...
describe("buildResolveConflictsPrompt", () => {
  ...
  it("points at the current checkout instead of asserting the local branch name", () => {
    // Fork threads check the PR out under `vulcan/pr-N/<branch>`, so the prompt must not
