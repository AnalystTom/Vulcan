...
import { VulcanThreadCreationCard } from "./VulcanThreadCreationCard";
...
interface MessagesTimelineProps {
  ...
  /** Provenance for a conversation created from another Vulcan task. */
    ...
    >
      {row.kind === "work" &&
        (() => {
          ...
          // The provider's actual Vulcan MCP tool rows remain visible here.
          const groupedEntries = row.groupedEntries.filter(
            (workEntry) => !workEntry.vulcanThreadCreation,
          );
            ...
            <div className="flex w-full flex-col gap-3">
              ...
              <div className="flex w-full justify-end">
                ...
                >
                  ...
                  {/* The cross-task origin label already attributes this turn to another Vulcan thread,
      ...
      {row.kind === "message" &&
        ...
        (() => {
          ...
          const buildWorkDisplay = (workEntries: WorkLogEntry[], workGroupId: string | null) => {
            const displayEntries = workEntries.filter((entry) => !entry.vulcanThreadCreation);
          ...
          const vulcanThreadCreationRecaps = [
            ...new Map(
              allTurnWorkEntries.flatMap((entry) =>
                entry.vulcanThreadCreation
                  ? [[entry.vulcanThreadCreation.operationId, entry.vulcanThreadCreation] as const]
                  : [],
              ),
            ).values(),
          ];
          const collapsedTurnItems = row.collapsedTurnItems?.filter(
            (item) => item.kind !== "work" || !item.entry.vulcanThreadCreation,
          );
            ...
            <>
              ...
              <div className="group min-w-0 py-0.5">
                ...
                {!row.assistantTurnInProgress && row.showAssistantCopyButton
                  ? vulcanThreadCreationRecaps.map((creation) => (
                      <div key={creation.operationId} className="mt-2 mb-4">
                        <VulcanThreadCreationCard
                          creation={creation}
                          {...(onOpenThread
                            ? {
