// FILE: VulcanThreadCreationCard.tsx
// Purpose: End-of-turn recap for threads created through the Vulcan MCP harness.
// Layer: Chat transcript UI

import { PROVIDER_DISPLAY_NAMES } from "@vulcan/contracts";
import { formatModelDisplayName } from "@vulcan/shared/model";
import { memo } from "react";

import type { WorkLogVulcanThreadCreation } from "../../session-logic";
import { ProviderIcon } from "../ProviderIcon";
import { VulcanLogo } from "../VulcanLogo";
import { Button } from "../ui/button";

function threadMeta(thread: WorkLogVulcanThreadCreation["threads"][number]): string {
...
export const VulcanThreadCreationCard = memo(function VulcanThreadCreationCard({
  creation,
  onOpenThread,
}: {
  readonly creation: WorkLogVulcanThreadCreation;
  ...
  return (
    <div
      ...
      data-vulcan-thread-creation-card="true"
    >
      <div className="flex min-w-0 items-center gap-3 px-3 py-2.5">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-background-elevated-secondary)] text-foreground">
          <VulcanLogo className="h-[22px] w-auto" aria-label="Vulcan" />
        </span>
