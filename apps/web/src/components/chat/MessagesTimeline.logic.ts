...
import { type MessageId, type TurnId } from "@vulcan/contracts";
...
function workLogVulcanThreadCreationsEqual(
  a: WorkLogEntry["vulcanThreadCreation"],
  b: WorkLogEntry["vulcanThreadCreation"],
) {
  if (a === b) return true;
  if (!a || !b) return false;
  if (
...
function workLogEntryContentEqual(a: WorkLogEntry, b: WorkLogEntry): boolean {
  return (
    ...
    workLogVulcanThreadCreationsEqual(a.vulcanThreadCreation, b.vulcanThreadCreation) &&
