...
// Purpose: Identify the source thread for conversations created by another Vulcan agent.
// Layer: Chat transcript UI

import { type ProviderKind, type ThreadId } from "@vulcan/contracts";
...
// Vulcan thread, so it always reads as "Sent by Vulcan" with the Vulcan mark
// (the origin provider is not surfaced here to keep one consistent label).
function OriginContent(): ReactNode {
  return (
    <>
      <span className="flex size-4 shrink-0 items-center justify-center text-muted-foreground/70">
        <VulcanLogo className="h-4 w-auto" aria-label="Vulcan" />
      </span>
      <span className="truncate">Sent by Vulcan from another thread</span>
    </>
  );
}
