...
import type { TurnId } from "@vulcan/contracts";
import {
  createElement,
  memo,
  useCallback,
...
const VulcanToolIcon: LucideIcon = ({ className, ...props }) => (
  <VulcanLogo {...props} className={cn("text-current", className)} />
);
...
export function workEntryLeftIcon(workEntry: TimelineWorkEntry): LucideIcon {
  ...
  if (isVulcanBrowserWorkEntry(workEntry)) return GlobeIcon;
  if (isVulcanToolCall(workEntry)) return VulcanToolIcon;
...
// Vulcan's own agent-gateway tools (vulcan_list_threads, vulcan_create_thread,
// ...) get the Vulcan mark instead of the generic MCP glyph. Providers report
// the call differently: Claude prefixes the MCP server (mcp__vulcan__*), ACP
// agents surface the bare tool name (vulcan_*), and Codex reports server/tool
// pairs that the label humanizer renders as "Vulcan: ...".
function toolWorkEntryStatus(workEntry: TimelineWorkEntry): VulcanMcpToolStatus {
...
function isVulcanBrowserWorkEntry(workEntry: TimelineWorkEntry): boolean {
  return isVulcanBrowserToolCall({
...
function isVulcanToolCall(workEntry: TimelineWorkEntry): boolean {
  return (
    deriveVulcanMcpToolTitle({
      toolName: workEntry.toolName,
      title: workEntry.toolTitle,
...
function toolWorkEntryHeading(workEntry: TimelineWorkEntry): string {
  const vulcanTitle = deriveVulcanMcpToolTitle({
    toolName: workEntry.toolName,
    title: workEntry.toolTitle,
    fallbackLabel: workEntry.label,
    status: toolWorkEntryStatus(workEntry),
...
export const TimelineWorkEntryRow = memo(function TimelineWorkEntryRow(props: {
  ...
  const isVulcanBrowserToolRow = !isGitHubToolRow && isVulcanBrowserWorkEntry(workEntry);
  const isVulcanToolRow =
    !isGitHubToolRow && !isVulcanBrowserToolRow && isVulcanToolCall(workEntry);
  const isMcpToolRow =
    workEntry.itemType === "mcp_tool_call" &&
    !isGitHubToolRow &&
    !isVulcanBrowserToolRow &&
    !isVulcanToolRow;
  ...
  const preview =
    isVulcanBrowserToolRow || isVulcanToolRow
      ? sanitizeVulcanMcpToolPreview({
          preview: rawPreview,
          heading,
          status: toolWorkEntryStatus(workEntry),
