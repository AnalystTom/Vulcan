...
// Purpose: A once-a-second clock for surfaces whose numbers grow while a run is live.
// Layer: Workspace UI
//
// Shared so the timeline and the run strip advance on the same tick: two clocks
 ...
 * every render, so a finished run stops invalidating memos.
 */
