...
export function BotWorkGraph({ data }: { data: BotListResult }) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-5 py-4">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Live task ownership across your standing team.
          </p>
        </div>
        ...
        <div className="relative min-h-[430px] overflow-x-auto bg-[radial-gradient(circle_at_center,hsl(var(--muted))_0,transparent_64%)]">
          ...
          <div className="relative h-[430px] min-w-[680px]">
            {botNodes.map((node) => {
              ...
              return <BotNode key={node.id} bot={bot} phase={phase} position={position} />;
            })}
            {taskNodes.map((node) => {
              const position = positions.get(node.id);
