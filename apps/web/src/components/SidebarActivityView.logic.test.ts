...
describe("project filter", () => {
  ...
  it("merges every project-less chat container into one Vulcan scope", () => {
...
describe("resolveActivityScope", () => {
  ...
  it("expands the Vulcan chats scope to its container projects", () => {
...
describe("resolveThreadProjectLabel", () => {
  it("uses the project name for real projects and Vulcan otherwise", () => {
    expect(
      resolveThreadProjectLabel({ kind: "project", name: "Vulcan App", folderName: "vulcan" }),
    ).toBe("Vulcan App");
    expect(resolveThreadProjectLabel({ kind: "chat", name: "Chats", folderName: "chats" })).toBe(
      "Vulcan",
    );
    expect(resolveThreadProjectLabel(undefined)).toBe("Vulcan");
  });
});
