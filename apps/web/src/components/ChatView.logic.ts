...
import { normalizeModelSlug } from "@vulcan/shared/model";
import { buildVulcanBranchName } from "@vulcan/shared/git";
import { isGenericChatThreadTitle } from "@vulcan/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@vulcan/shared/terminalThreads";
import {
  type ChatMessage,
  type SessionPhase,
  type Thread,
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "vulcan:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "vulcan:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for Vulcan, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildVulcanBranchName(input.associatedWorktreeBranch ?? input.title);
}
