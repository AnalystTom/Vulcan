...
import type { BrowserAnnotationEvent, ThreadBrowserState, ThreadId } from "@vulcan/contracts";
...
const pipePath = process.env.VULCAN_BROWSER_HOST_PIPE_PATH;
const capability = process.env.VULCAN_BROWSER_HOST_CAPABILITY;
const shellPath = process.env.VULCAN_E2E_SHELL_PATH;
const threadId = process.env.VULCAN_E2E_THREAD_ID as ThreadId | undefined;
const vulcanHome = process.env.VULCAN_HOME;
const annotationPreloadPath = process.env.VULCAN_E2E_BROWSER_ANNOTATION_PRELOAD;

if (!pipePath || !capability || !shellPath || !threadId || !vulcanHome || !annotationPreloadPath) {
  throw new Error("The visible-browser Electron fixture requires its isolated E2E environment.");
}

app.setPath("userData", path.join(vulcanHome, "electron-userdata"));
...
function setPanelVisible(visible: boolean): void {
  ...
  pushState();
  mainWindow?.webContents.send("vulcan-e2e:open-panel");
}
function pushState(): void {
  if (shellReady && latestState && mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("vulcan-e2e:browser-state", latestState);
  }
}
...
ipcMain.on("vulcan-e2e:shell-ready", () => {
  shellReady = true;
  pushState();
});

ipcMain.handle(
  "vulcan-e2e:attach-webview",
