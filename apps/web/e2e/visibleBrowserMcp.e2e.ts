...
import { BROWSER_TOOL_NAMES } from "@vulcan/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "vulcan-visible-browser-e2e-"));
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __vulcanVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__vulcanVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__vulcanVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __vulcanVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
