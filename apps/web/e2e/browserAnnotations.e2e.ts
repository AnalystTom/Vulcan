...
import type { BrowserAnnotationEvent, BrowserAnnotationTheme } from "@vulcan/contracts";
...
test("a real Electron guest commits and reprojects a continuous annotation session", async () => {
  const mainPath = process.env.VULCAN_E2E_ELECTRON_MAIN;
  ...
  const home = mkdtempSync(join(tmpdir(), "vulcan-browser-annotations-e2e-"));
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      VULCAN_HOME: home,
      VULCAN_BROWSER_HOST_PIPE_PATH: pipePath,
      VULCAN_BROWSER_HOST_CAPABILITY: capability,
      VULCAN_E2E_SHELL_PATH: shellPath,
      VULCAN_E2E_THREAD_ID: threadId,
      VULCAN_E2E_BROWSER_ANNOTATION_PRELOAD: annotationPreloadPath,
    },
  });
  ...
  try {
    ...
    const sendNativeInput = (event: Record<string, unknown>) =>
      electronApp.evaluate(
        (_electron, input) => {
          ...
          ).__vulcanVisibleBrowserE2E.browserManager;
    ...
    const annotationEvents = async (): Promise<BrowserAnnotationEvent[]> =>
      electronApp.evaluate(() => {
        const fixture = (
          globalThis as typeof globalThis & {
            __vulcanVisibleBrowserE2E: { annotationEvents: BrowserAnnotationEvent[] };
          }
        ).__vulcanVisibleBrowserE2E;
