// FILE: vite.config.ts
// Purpose: Builds the Vulcan web client and controls diagnostic source maps.
...
const sourcemapEnv = process.env.VULCAN_WEB_SOURCEMAP?.trim().toLowerCase();
...
function centralIconPrunePlugin(): Plugin {
  ...
  return {
    name: "vulcan-central-icon-prune",
    apply: "build",
