...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `vulcan` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@vulcan/shared/git`, `@vulcan/shared/DrainableWorker`) — no barrel index.
